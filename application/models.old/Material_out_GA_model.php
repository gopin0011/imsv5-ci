<?php class Material_out_GA_model extends CI_Model{
...
$limit= 50; $offset = 0;
return $this->db->query("SELECT * FROM QTH_RawMaterial WHERE IDTrcType=230 ORDER BY RegID DESC OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY");
}
...
function MasterList(){
return $this->db->query("SELECT * FROM ListMaterialOut WHERE IDTrcType=130 ORDER BY RegID ASC"); }

function MasterList2(){
return $this->db->query("SELECT * FROM Q01_MProduct WHERE IsGA=1 AND IsActive=1 AND IsDelete='X' ORDER BY PartName DESC"); }
...
function transaction_detail_report($tgl1,$tgl2,$IDCategory,$ItemID,$IDDepartment){
 $where = "WHERE DocDate BETWEEN '$tgl1' AND '$tgl2' AND IDTrcType='230'" ; 
 if(empty($ItemID)){
 if($IDCategory!='semua'){
 if($IDDepartment!='semua'){
 $where .= "AND department='$IDDepartment' AND IDCategory='$IDCategory'";}else{
 $where .= "AND IDCategory='$IDCategory' ";}
 }else{
 if($IDDepartment!='semua'){
 $where .= "AND department='$IDDepartment'";} }
 }else{
 if($IDCategory!='semua'){
 if($IDDepartment!='semua'){
 $where .= "AND ItemID='$ItemID' AND department='$IDDepartment' AND IDCategory='$IDCategory'";}else{
 $where .= "AND ItemID='$ItemID' AND IDCategory='$IDCategory'";}
 }else{
 if($IDDepartment!='semua'){
 $where .= "AND ItemID='$ItemID' AND department='$IDDepartment'";}
