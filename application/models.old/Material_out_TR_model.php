<?php class Material_out_TR_model extends CI_Model{
...
$limit= 50; $offset = 0;
return $this->db->query("SELECT * FROM QTH_RawMaterial WHERE IDTrcType=210 ORDER BY RegID DESC OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY");
}
...
function MasterList(){
return $this->db->query("SELECT * FROM ListMaterialOut WHERE IDTrcType=110 ORDER BY RegID ASC"); }

function MasterList2(){
return $this->db->query("SELECT * FROM Q01_MProduct WHERE IsStoreRoom=1 AND IsActive=1 ORDER BY PartName DESC"); }
...
function transaction_detail_report($tgl1,$tgl2,$IDCategory,$ItemID,$IDDepartment){
 $where = "WHERE DocDate BETWEEN '$tgl1' AND '$tgl2' AND IDTrcType='210'" ; 
 if(empty($ItemID)){
 if($IDCategory!='semua'){
