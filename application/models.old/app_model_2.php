...
date_default_timezone_set('Asia/Jakarta');
  ...
  public function CariTotalACC($bln,$thn){
    	...
    	public function total_cust_item(){
		$t = "SELECT ItemID FROM MonitoringRM ";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
        ...
        public function cari_percent_danger(){
		$t = "SELECT hari FROM MonitoringRM WHERE (hari BETWEEN 0 AND 0.99)";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function cari_percent_warning(){
		$t = "SELECT hari FROM MonitoringRM WHERE (hari BETWEEN 1 AND 1.999)";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function cari_percent_safe(){
		$t = "SELECT hari FROM MonitoringRM WHERE (hari BETWEEN 3 AND 6.99)";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function cari_percent_warning_up(){
		$t = "SELECT hari FROM MonitoringRM WHERE (hari BETWEEN 7.01 AND 10)";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function cari_percent_danger_up(){
		$t = "SELECT hari, IDCust FROM dbo.MonitoringRM WHERE (hari > 10.01)";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function Nama_lengkap($id){
    	...
    	public function ItemMMaterial($id){
		$t = "SELECT RegID FROM M_Product WHERE IsMaterial='$id' AND IsActive='1'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function ItemMStamping($id){
		$t = "SELECT RegID FROM M_Product WHERE IsStamping='$id' AND IsActive='1'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function ItemMWelding($id){
		$t = "SELECT RegID FROM M_Product WHERE IsWelding='$id' AND IsActive='1'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function ItemMStoreRoom($id){
		$t = "SELECT RegID FROM M_Product WHERE IsStoreRoom='$id' AND IsActive='1'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function ItemMGA($id){
		$t = "SELECT RegID FROM M_Product WHERE IsGA='$id' AND IsActive='1'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function TotalItemMAsset(){
		$t = "SELECT count(SysID) as jml FROM M_Asset WHERE IsActive='1'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
		  foreach($d->result() as $h){
...
date_default_timezone_set('Asia/Jakarta');
		...
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
				$lastNoINS=$t->last;
                $lastNoUrut=substr($lastNoINS,15,4);
                $nextNoUrut=$lastNoUrut+1;
...
date_default_timezone_set('Asia/Jakarta');
    ...
    public function QtyRM_IN($kode,$tgl_awal,$tgl_akhir){
		$t = "SELECT ItemID,sum(Qty_1) as jml 
			FROM QTD_Trace WHERE ItemID='$kode' AND (DocDate BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."') AND TrcType=1000
			GROUP BY ItemID";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
    ...
    public function QtyRM_OUT($kode,$tgl_awal,$tgl_akhir){
		$t = "SELECT ItemID,sum(Qty_1) as jml FROM QTD_Trace  
		...
		public function getLoginData($usr,$psw)
	{
	   ...
	   $p = md5($psw);
		...
		$q_cek_login = $this->db->get_where('IMSUser', array('username' => $u, 'password' => $p ));
		if(count($q_cek_login->result())>0)
		{
			foreach($q_cek_login->result() as $qck)
			{
					...
					header('location:'.base_url().'index.php/Comment/');
			}
		}
		else
		{
			$this->session->set_flashdata('result_login', '<br>Password atau username salah.');
		...
		$q_cek_login = $this->db->get_where('IMSUser', array('username' => $u, 'password2' => $p ));
		if(count($q_cek_login->result())>0)
		{
			foreach($q_cek_login->result() as $qck)
			{
