...
public function CariUsername($id){
		$t = "SELECT * FROM M_User WHERE ActivationID='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			foreach($d->result() as $h){
...
/*
    public function ItemMICT($id,$id2)
    {
        $CategoryID = "" ;
        if($id2!='All'){$CategoryID .= "AND IDCategory='$id2'" ; }else{$CategoryID .= "" ; }
        $t = "SELECT RegID FROM M_Product WHERE IsICT='$id' AND IsActive='1' $CategoryID ";
        $d = $this->app_model->manualQuery($t);
        $r = $d->num_rows();
        if($r>0)
        {
...
date_default_timezone_set('Asia/Jakarta');
    ...
    /*{{}}*/
    public function DocNumIctOUTRMOther(){
        //Time SERVER,
date_default_timezone_set('Asia/Jakarta');
        $today="OUTICT".date('ymd');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
