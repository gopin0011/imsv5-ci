<?php class Material_report_GA_model extends CI_Model{
    ...
    public function CariShift($id){
		...
		$d = $this->Material_report_GA_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
    ...
    public function tgl_indo($tgl){
			...
			$bulan = $this->Material_report_GA_model->getBulan(substr($tgl,5,2));
...
function MasterList(){
 $text = "SELECT * FROM Q01_MProduct WHERE IsGA=1 AND IsActive=1 ORDER BY IDCategory DESC";    
...
function transaction_detail_report($IDCategory,$part_no,$spec){
 ...
 $text = "SELECT * FROM StockGA $where ORDER BY PartNo ASC ";
 return $this->db->query($text);  }
...
function ReadStockCard($tgl1,$tgl2,$ItemID){  
 ...
 $text = "SELECT* FROM SCGA $where ORDER BY DocDate, DocTime ASC ";
 return $this->db->query($text); }
