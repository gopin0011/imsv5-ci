...
$IN  = $this->ReportMaterial_model->QtyRM_IN($row->ItemID,$tgl_awal,$tgl_akhir);
$OUT  = $this->ReportMaterial_model->QtyRM_OUT($row->ItemID,$tgl_awal,$tgl_akhir);
$RET  = $this->ReportMaterial_model->QtyRM_RET($row->ItemID,$tgl_awal,$tgl_akhir);
$IN_2  = $this->ReportMaterial_model->QtyRM_IN($row->ItemID,$tgl_akhir_2,$tgl_sekarang2);
$OUT_2  = $this->ReportMaterial_model->QtyRM_OUT($row->ItemID,$tgl_akhir_2,$tgl_sekarang2);
$RET_2  = $this->ReportMaterial_model->QtyRM_RET($row->ItemID,$tgl_awal,$tgl_sekarang2);
$IN_F  = $this->ReportMaterial_model->QtyRM_IN($row->ItemID,$tgl_awal,$tgl_akhir);
$OUT_F  = $this->ReportMaterial_model->QtyRM_OUT($row->ItemID,$tgl_awal,$tgl_akhir);
$RET_F = $this->ReportMaterial_model->QtyRM_RET($row->ItemID,$tgl_awal,$tgl_akhir);
...
<?php echo $row->Spec1 .' '. $row->Spec2  ; ?></td>
...
<i style="cursor:pointer" class="glyphicon glyphicon-edit" target="popup" onclick="window.open('<?php echo base_url(); ?>index.php/ReportMaterial/DataRecord/<?php echo $row->ItemID ;?>','name','width=1200,height=600')"></i>
<?php } ?>
</td>
</tr>
<?php endforeach;?>
