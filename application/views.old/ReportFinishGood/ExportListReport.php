...
foreach($list as $row){  
 ...
 if($StockDay>=1 && $StockDay<2){ $bar="Warning"; }
 if($StockDay>=2 && $StockDay<=3){ $bar="Safe"; }
 if($StockDay>3 && $StockDay<=5){ $bar="Caution"; }
 if($StockDay>5){ $bar="Over"; }  
 ...
 <td align="left" width="80" ><?php echo number_format($FG_IN); ?> </td>
 <td align="left" width="80" ><?php echo number_format($FG_OUT); ?> </td>
...
header("Content-Disposition: attachment; filename=FGStock_$periode.xls");
