<table id="t_list_master2" class="table table-bordered table-striped">
<thead>
<tr class="headings">
    <th>PartNo</th>  
    <th>PartName</th>  
    <th>Cust</th> 
    <th>Project</th>
</tr> 
</thead>
<tbody>
<?php $no=0; foreach($MListProduct2 as $row): 
$no++; 
$PartNo = substr($row->PartNo,0,15) ;
$PartName = substr($row->PartName,0,20) ;
?>    
<tr class="odd gradeX" style="cursor:pointer" onclick="javascript:pilih2('<?php echo $row->RegID;?>')">
<td width="150"><?php echo $PartNo ; ?></td>
<td width="250"><?php echo $PartName ; ?></td>
<td width="180"><?php echo $row->CustName ; ?></td>          
<td align="center" width="180" ><?php echo $row->ProjectName ; ?></td>
</tr>
<?php endforeach;?>
</tbody>
</table>

    <script type="text/javascript"> 
function pilih2(id){
	$("#myModal_MaterialList2").modal('hide');
	$("#ItemID22").val(id);
	$("#ItemID22").focus(); }
</script>

    
    <script> $(function () { $("#t_list_master2").DataTable(); });</script>