...
class BPFG_in extends CI_Controller{
    ...
    {
        parent::__construct();
        $this->load->model(array('BPFG_in_model','app_model','Role_Model'));
    ...
    function TransactionListProductionIn()
    {
        ...
        $sql = "SELECT a.*, dbo.fn_newDocNum(a.RegID) as NewDocNum
                ...
                WHERE a.IDTrcType=605
                ORDER BY RegID DESC OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
        $sql = "SELECT b.DocNum,b.DocDate,b.DocTime,b.RegID,c.username AS CreateBy, a.TotalItem AS TotalDetail,dbo.fn_newDocNum(b.RegID) as NewDocNum
                FROM
                (
                    SELECT ab.DocNum, COUNT(ab.ItemID) AS TotalItem
					FROM
					(
						SELECT a.ItemID,a.RegID,a.DocNum,a.QtyMat AS QtyMatA,SUM(ISNULL(b.QtyMat,0)) as QtyMatB
						FROM
						(
							SELECT bb.ItemID,bb.RegID,SUM(bb.QtyMat) AS QtyMat,aa.DocNum
							FROM TH_RawMaterial aa
							JOIN TD_RawMaterial bb ON bb.DocNum = aa.DocNum
							WHERE aa.IDTrcType=605 AND bb.IsDelete = 'X'
								-- AND aa.DocNum = 'BPFG170524026'
								-- AND bb.ItemID = 2011
							GROUP BY bb.ItemID,bb.RegID,aa.DocNum,aa.RegID
						) a
						LEFT JOIN TD_Transdetail b ON b.TD_RawMaterial = a.RegID AND b.ItemID = a.ItemID AND b.IsDelete = 0	
						GROUP BY a.ItemID,a.RegID,a.DocNum,a.QtyMat
						HAVING SUM(ISNULL(b.QtyMat,0)) < a.QtyMat
					) ab
					GROUP BY ab.DocNum
                ) a
                JOIN TH_RawMaterial b ON b.DocNum = a.DocNum
                JOIN M_User c ON c.RegID = b.CreateBy
                ORDER BY a.DocNum DESC OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
        //echo"<pre>";
        //print_r($sql);
        //echo"</pre>";
    ...
    function DetailTrc()
    {
        ...
        $sql = "SELECT bb.ItemID,bb.RegID,bb.PartNo,bb.PartName,bb.Code,(bb.QtyMat-ISNULL(aa.QtyMat,0)) as QtyMat, bb.Remark, bb.Location AS FromArea 
                FROM
                ...
                RIGHT JOIN 
                    (
                        ...
                        LEFT JOIN M_Location c ON b.FromArea = c.SysID
                        WHERE a.IDTrcType=605 AND a.RegID = '$RegID' 
        ...
        $data['list']=$this->app_model->dbQuery($sql)->result();
        if($this->app_model->dbQuery($sql)->num_rows()>0)
        {
            foreach($data['list'] as $row)
            {
                $data['dari_area'] = $row->FromArea;
            }
        }
        else $data['dari_area'] = '';
		...
		$text6 = "SELECT * FROM M_Shift2 WITH (NOLOCK)";
		$data['M_Shift'] = $this->app_model->manualQuery($text6);
        
        $sql_location = "SELECT * FROM M_Location WHERE Category NOT LIKE 'ASSET' ORDER BY Category,Location";
        $data['area'] = $this->app_model->dbQuery($sql_location)->result();
        ...
        if($ok)
        {
            foreach($data['dbRegId'] as $key => $v)
            {
                ...
                if($data['QtyMat'][$v]>0)
                {
					...
					{
						$Stock = $this->app_model->db_get_one("SELECT StockFG FROM M_Product WHERE RegID='".$data['ItemID'][$v]."'");
						$NewStock = floatval($Stock) + floatval($data['QtyMat'][$v]);
						$this->app_model->updateData("M_Product",array('StockFG'=>$NewStock),array('RegID'=>$data['ItemID'][$v]));
					}
                }
            }
        }
    ...
    function get_header_trans()
    {
        ...
        $data['diserahkan'] = array($q->id,$q->nama_lengkap,$q->from_area);
        $to_area = $this->app_model->db_get_one("SELECT Location FROM M_Location WHERE SysID='".$q->to_area."'");
        $byID = $this->session->userdata('RegID');
        $sql = "SELECT b.nama_lengkap,b.area,b.RegID as id FROM M_User b WHERE b.RegID='".$byID."'";
        $q = $this->app_model->dbQuery($sql)->row();
    ...
    function hapus_trans_detail()
    {
        ...
        if($ok)
        {
			$Stock = $this->app_model->db_get_one("SELECT b.StockFG FROM TD_Transdetail a JOIN M_Product b ON b.RegID = a.ItemID WHERE a.RegID='".$RegID."'");
			$QtyMat = $this->app_model->db_get_one("SELECT QtyMat FROM TD_Transdetail WHERE RegID='".$RegID."'");
			$NewStock = floatval($Stock) - floatval($QtyMat);
			
			$ItemID = $this->app_model->db_get_one("SELECT b.RegID FROM TD_Transdetail a JOIN M_Product b ON b.RegID = a.ItemID WHERE a.RegID='".$RegID."'");
			$this->app_model->updateData("M_Product",array('StockFG'=>$NewStock),array('RegID'=>$ItemID));			
            echo "Delete Sukses";
        } 
        else 
        {
    ...
    {
        $RegID = $this->input->post('RegID');
		$QtyMat = $this->app_model->db_get_one("SELECT a.QtyMat FROM TD_Transdetail a WHERE a.RegID ='".$RegID."'");
        ...
        {
			$Stock = $this->app_model->db_get_one("SELECT b.StockFG FROM TD_Transdetail a JOIN M_Product b ON b.RegID = a.ItemID WHERE a.RegID ='".$RegID."'");
			$NewStock = ($Stock - $QtyMat) + $data['QtyMat'];
			$ItemID = $this->app_model->db_get_one("SELECT b.RegID FROM TD_Transdetail a JOIN M_Product b ON b.RegID = a.ItemID WHERE a.RegID ='".$RegID."'");
			$this->app_model->updateData("M_Product",array('StockFG'=>$NewStock),array('RegID'=>$ItemID));
            echo "Data Terupdate";
        }
    ...
    function ReportBPFG()
    {
        ...
        $FromArea = $this->input->post('FromArea_Search');
        
        $where = ($IDCust != 'semua') ? " AND c.IDCust = '".$IDCust."'" : '';
        $where .= ($PartNo2!='') ? " AND c.PartNo LIKE '%".$PartNo2."%'" : ''; 
        $where .= ($FromArea!='') ? " AND a.from_area = '".$FromArea."'" : '';
        $where .= " AND a.DocDate BETWEEN '".$tgl1."' AND '".$tgl2."' ";
        $sql = "SELECT SUM(b.QtyMat) as total, c.PartNo, c.PartName, e.Code, MAX(c.StockFG) as StockFG
                FROM TH_Trans a
    ...
    function SearchDocNum()
    {
        $DocNum = $this->input->post('DocNumDetail2');
        //$DocNum = 'BPFG170421002';
        /*
        $sql = "SELECT a.*, dbo.fn_newDocNum(a.RegID) as NewDocNum
                FROM QTH_RawMaterial a 
                WHERE a.IDTrcType=605 AND dbo.fn_row_detail_bpfg(a.RegID) > 0 
                    AND a.DocNum LIKE '%$DocNum%'
                ORDER BY RegID DESC OFFSET 0 ROWS FETCH NEXT 50 ROWS ONLY";
                */
        $sql = "SELECT b.DocNum,b.DocDate,b.DocTime,b.RegID,c.username AS CreateBy, a.TotalItem AS TotalDetail,dbo.fn_newDocNum(b.RegID) as NewDocNum
                FROM
                (
        ...
        if($rs->num_rows()>0)
        {
            ...
            foreach($data['list'] as $row)
            {
                $row->DocNum = $this->InfoTambahFormTRMatIn($row->RegID,$row->NewDocNum)['DocNum'];
                $row->DocNumDetail = $this->InfoTambahFormTRMatIn($row->RegID,$row->NewDocNum)['DocNumDetail'];
            }
            //$data['layout'] = $this->load->view('BPFG_in/DetailTrc',$data, true);
        }
        else
        {
            $data['list'] = array();
