<?php
class Sony_in extends CI_Controller{
function __construct(){
 parent::__construct();
 $this->load->model(array('Sony_in_model','app_model'));
 $this->load->library(array('form_validation','template'));
 $cek = $this->Role_Model->TrcSony();
...
function index(){
 ...
 $text = "SELECT * FROM Master_BOM WITH (NOLOCK) WHERE CustName LIKE '%SONY%' AND IsActiveDetail=1 AND PartTypeID LIKE '%RM%' ORDER BY SysID ASC" ;
...
$d['DocDateReport_2']= $DocDate ;
        $this->template->display('Sony_in/index',$d); }
    ...
    function transaction_list(){
        $cek=$this->Sony_in_model->transaction_list();
        $data['list']=$cek->result();
        $this->load->view('Sony_in/transaction_list',$data);
    }
    
    function transaction_detail(){
        $id = $this->input->post('id');
        $cek =$this->Sony_in_model->transaction_detail($id);
        $data['list']=$cek->result();
        $this->load->view('Sony_in/transaction_detail',$data);
    ...
    function print_list(){
        ...
        $this->load->view('Sony_in/PrintList',$data);
    ...
    function export_list(){
        ...
        $id = $this->uri->segment(3);
			...
			if($data2->num_rows() > 0){
				foreach($data2->result() as $db){
                    ...
                    $DocDate2 = $this->Sony_in_model->tgl_str($db->DocDate) ;
					$data['DocNum']	= $id ;
					$data['DocDate']	= $this->app_model->tgl_str($db->DocDate);
                    $data['Shift']	= $db->Shift;
        ...
        $data['num'] = $cek->num_rows();
        $this->load->view('Sony_in/ExportList',$data);
    ...
    function transaction_detail_2(){
        ...
        $cek =$this->Sony_in_model->transaction_detail($id);
        $data['list']=$cek->result();
        $this->load->view('Sony_in/transaction_detail_2',$data);
    ...
    function export_list_report(){
        ...
        $cek = $this->Sony_in_model->transaction_detail_report($DocDateReport_1,$DocDateReport_2,$RefNumReport,$ShiftIDReport);
        $d['data']=$cek->result();  
        $d['num'] = $cek->num_rows();   
        $this->load->view('Sony_in/ExportListReport',$d);
    ...
    public function ReadReport(){
            ...
            $cek = $this->Sony_in_model->transaction_detail_report($DocDateReport_1,$DocDateReport_2,$RefNumReport,$ShiftIDReport);
            $Stock = $this->Sony_in_model->Stock_List();
            $d['list']=$cek->result();  
            $d['Stock']=$Stock->result();             
			$this->load->view('Sony_in/transaction_detail_report',$d);
	}
    ...
    public function print_list_report(){
          ...
          $d['Shift'] = "ALL Shift";
            ...
            $d['data']=$cek->result();  
            $d['num'] = $cek->num_rows();           
			$this->load->view('Sony_in/PrintListReport',$d);
	}
...
date_default_timezone_set('Asia/Jakarta');	
          ...
          echo json_encode($data);
			...
			foreach($tabel->result() as $t){
                ...
                $data['DocDate'] = $this->Sony_in_model->tgl_str($t->DocDate);
...
date_default_timezone_set('Asia/Jakarta');
            ...
            $ItemID_Ext = substr($this->input->post('SysID'),0,10);
                ...
                $detail['Qty_1'] = '1' ; 
                $detail['Qty_2'] = $this->input->post('Qty_2') ; 
                $detail['Qty_3'] = $this->input->post('Qty_2') ;
                ...
                $detail2['Qty_2'] = $this->input->post('Qty_2') ; 
                $detail2['Qty_3'] = $this->input->post('Qty_2') ;
                ...
                $id_head_id['DocNum_Ext'] = $this->input->post('RefNum');
                
                $id_head_id2['DocNum_Ext'] = $this->input->post('RefNum');
