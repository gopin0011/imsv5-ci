<?php
class Sony_prod extends CI_Controller{
function __construct(){
 parent::__construct();
 $this->load->model(array('Sony_prod_model','app_model'));
...
$d['DocDateReport_2']= $DocDate ;
        $this->template->display('Sony_prod/index',$d);
    }
    ...
    function transaction_list(){
        $cek=$this->Sony_prod_model->transaction_list();
        $data['list']=$cek->result();
        $this->load->view('Sony_prod/transaction_list',$data);
    }
    
    function transaction_detail(){
        $id = $this->input->post('id');
        $cek =$this->Sony_prod_model->transaction_detail($id);
        $data['list']=$cek->result();
        $this->load->view('Sony_prod/transaction_detail',$data);
    ...
    function print_list(){
        ...
        $this->load->view('Sony_prod/PrintList',$data);
    ...
    function export_list(){
        ...
        $id = $this->uri->segment(3);
			...
			if($data2->num_rows() > 0){
				foreach($data2->result() as $db){
                    ...
                    $DocDate2 = $this->Sony_prod_model->tgl_str($db->DocDate) ;
					$data['DocNum']	= $id ;
					$data['DocDate']	= $this->app_model->tgl_str($db->DocDate);
                    $data['Shift']	= $db->Shift;
        ...
        $data['num'] = $cek->num_rows();
        $this->load->view('Sony_prod/ExportList',$data);
    ...
    function transaction_detail_2(){
        ...
        $cek =$this->Sony_prod_model->transaction_detail($id);
        $data['list']=$cek->result();
        $this->load->view('Sony_prod/transaction_detail_2',$data);
    ...
    function export_list_report(){
        ...
        $cek = $this->Sony_prod_model->transaction_detail_report($DocDateReport_1,$DocDateReport_2,$RefNumReport,$ShiftIDReport);
        $d['data']=$cek->result();  
        $d['num'] = $cek->num_rows();   
        $this->load->view('Sony_prod/ExportListReport',$d);
    ...
    public function ReadReport(){
            ...
            $cek = $this->Sony_prod_model->transaction_detail_report($DocDateReport_1,$DocDateReport_2,$RefNumReport,$ShiftIDReport);
            $d['list']=$cek->result();             
			$this->load->view('Sony_prod/transaction_detail_report',$d);
	}
    ...
    public function print_list_report(){
          ...
          $d['Shift'] = "ALL Shift";
            ...
            $d['data']=$cek->result();  
            $d['num'] = $cek->num_rows();           
			$this->load->view('Sony_prod/PrintListReport',$d);
	}
    ...
    public function RegDocNumSony_Head(){
          ...
          if(empty($id)){
            $DocNumDetail	        = '0001'   ; 
            $DocNum                 = $this->app_model->DocNumProdSony();
            }else{
...
date_default_timezone_set('Asia/Jakarta');	
          ...
          echo json_encode($data);
			...
			foreach($tabel->result() as $t){
                ...
                $data['DocDate'] = $this->Sony_prod_model->tgl_str($t->DocDate);
...
date_default_timezone_set('Asia/Jakarta');
            ...
            $Balance1 = $this->Sony_prod_model->CariBalanceProd($ID);
            $DocNum_Ext = $this->Sony_prod_model->CariTrcType($ID);
            $Balance = $Balance1 - 1 ;
