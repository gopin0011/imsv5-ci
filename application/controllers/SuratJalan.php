...
class SuratJalan extends CI_Controller{
 function __construct()
 {
    ...
    $cek = $this->Role_Model->TrcSJConfirm();
 ...
 function PrintList2()
 {
    ...
    $d['TotalQty'] = $this->app_model->dbQuery($sql_qty)->row()->total;
    $d['RegID'] = $RegID;
    $d['IsConfirm'] = $this->app_model->db_get_one("SELECT StatusConfirm FROM TH_SuratJalan WHERE RegID='$RegID'");
    $d['KetStatus'] = $this->app_model->db_get_one("SELECT KetStatus FROM TH_SuratJalan WHERE RegID='$RegID'");
 ...
 function PrintListReport()
 {
        ...
        $num = 0;
        $where = "AND b.DocDate BETWEEN '$tgl1' AND '$tgl2' AND b.TypeID NOT IN (1)" ; 
        if(empty($IDCust2))
        {
    ...
    $this->load->view('SuratJalan/PrintListReportNEW',$d);
 }
 ...
 function DataDetailMatIn2()
 {
    ...
    if($DB->num_rows()>0)$this->load->view('SuratJalan/DetailMatIn2',$data);
 }
 ...
 function GetData()
 {
    ...
    if($this->app_model->dbQuery($sql)->num_rows()>0) $this->load->view('SuratJalan/HeaderDetail',$data);
 }
 ...
 function GetData2()
 {
    ...
    function ReadReport()
    {
        ...
        //print_r($text);
        //echo"</pre>";
        $this->load->view('SuratJalan/transaction_detail_report_new',$d);
    }
    ...
    function update_confirm()
    {
     ...
     function GetTemplatesConfirm()
     {
        ...
        if($type == 2) 
        {
            $data['Keterangan'] = $this->app_model->db_get_one("SELECT KetStatus FROM TH_SuratJalan WHERE RegID='$RegID'");
            $d['templates'] = $this->load->view('SuratJalan/UnConfirm',$data,true);
        }
        else
        {
            $data['Driver'] = $this->app_model->db_get_one("SELECT DriverName FROM TH_SuratJalan WHERE RegID='$RegID'");
            $data['CarNum'] = $this->app_model->db_get_one("SELECT CarNum FROM TH_SuratJalan WHERE RegID='$RegID'");
            $d['templates'] = $this->load->view('SuratJalan/Confirm',$data,true);
        } 
        $d['surjal'] = $this->app_model->db_get_one("SELECT DocNum FROM TH_SuratJalan WHERE RegID='$RegID'");
        echo json_encode($d);
     }
     
     function SaveConfirmStatus()
     {
        $RegID = $this->input->post('RegID');
        $type = $this->input->post('type');
        ...
        if($type == 2)
        {
            $data['KetStatus'] = $this->input->post('Remark');
            $data['StatusConfirm'] = 2;
            $data['IsConfirm'] = 0;
        } else {
        ...
        $data['ConfirmBy'] = $this->session->userdata('SysID');
        $data['ConfirmDate'] = date('Y-m-d');
        $data['ConfirmTime'] = date('H:i:s');
        
        $ok = $this->app_model->updateData("TH_SuratJalan",$data,array('RegID'=>$RegID));
        if($ok) echo "Update Sukses";
        else echo "Gagal mengupdate";
     }
     
     function GetAllCarNum()
     {
        	$sql = "SELECT CarNum FROM TH_SuratJalan WHERE CarNum LIKE '%".$_GET['query']."%' GROUP BY CarNum"; 
            $result = $this->app_model->dbQuery($sql);
     ...
     {
        $sql = "SELECT DriverName FROM TH_SuratJalan WHERE DriverName LIKE '%".$_GET['query']."%' GROUP BY DriverName"; 
        $result = $this->app_model->dbQuery($sql);
