<?php
class OutSTP extends CI_Controller{
 function __construct(){
 parent::__construct();
 $this->load->model(array('OutSTP_model','app_model'));
 $this->load->library(array('form_validation','template'));
 $cek = $this->session->userdata('TrcStamping')=='1';
...
function index(){
 ...
 $text2 = "SELECT * FROM M_Line WITH (NOLOCK) WHERE Category='STP' ";
...
$this->template->display('OutSTP/index',$d); }
...
public function PrintLabel(){
 ...
 $d['DocNum'] =$id;
 $d['DocDate'] =''; } 
 $DB=$this->OutSTP_model->transaction_detail_3($id);
 $d['data']=$DB->result();				
 $this->load->view('OutSTP/PrintLabel',$d);     	}
    
function MasterList(){
 $DB=$this->OutSTP_model->MasterList();
 $data['MListProduct']=$DB->result();
 $this->load->view('OutSTP/master_list',$data); }    

function MasterListPartner(){
 $DB=$this->OutSTP_model->MasterListPartner();
 $data['list']=$DB->result();
 $this->load->view('OutSTP/master_partner',$data); }
    
function TransactionList(){
 $DB=$this->OutSTP_model->transaction_list();
 $data['list']=$DB->result();
 $this->load->view('OutSTP/TransactionList',$data); }
    
function DataDetailMatIn(){
 $id = $this->input->post('kode');
 $DB =$this->OutSTP_model->transaction_detail($id);
 $data['list']=$DB->result();
 $this->load->view('OutSTP/DetailMatIn',$data); }
    
function DataDetailMatIn2(){
 $id = $this->input->post('kode');
 $DB =$this->OutSTP_model->transaction_detail($id);
 $data['list']=$DB->result();
 $this->load->view('OutSTP/DetailMatIn2',$data); }
 
function DataDetailMatIn3(){
 $id = $this->input->post('kode');
 $DB =$this->OutSTP_model->transaction_detail($id);
 $data['list']=$DB->result();
 $this->load->view('OutSTP/DetailMatIn3',$data); }
...
public function ReadReport(){
 ...
 $DB = $this->OutSTP_model->transaction_detail_report($tgl1,$tgl2,$IDCust,$Factory,$PartNo,$IDLine);
 $d['list']=$DB->result();              
 $this->load->view('OutSTP/transaction_detail_report',$d); }
    
public function ExportReport(){
 $tgl1 = $this->app_model->tgl_sql($this->uri->segment(3));
 $tgl2 = $this->app_model->tgl_sql($this->uri->segment(4));
 $Factory = $this->uri->segment(5);
 ...
 $d['data']= $DB->result(); 
 $d['num'] = $DB->num_rows();                
 $this->load->view('OutSTP/ExportListReport',$d); }
 ...
 $d['data']=$DB->result(); 
 $d['num'] = $DB->num_rows();                
 $this->load->view('OutSTP/PrintReport',$d); }
...
public function DetailPrint(){
 ...
 $DB =$this->OutSTP_model->transaction_detail($id);
 $d['data']=$DB->result();					
 $this->load->view('OutSTP/PrintMaterialIn',$d);   }
...
public function DetailPrint2(){
 ...
 $DB =$this->OutSTP_model->transaction_detail_2($id);
 $d['data']=$DB->result();					
 $this->load->view('OutSTP/PrintMaterialIn',$d);   }
...
public function Save(){
 ...
 $Header['ShiftID'] = $this->input->post('ShiftID') ;
 $Header['IDTrcType'] = '1000'; 
 $Header2['ItemID'] = $this->input->post('ItemID') ;
 $Header2['ProsesH'] = $this->input->post('ProsesH') ;
 $Header2['ShiftID'] = $this->input->post('ShiftID') ;
 $Header2['DocDate'] = $this->app_model->tgl_sql($this->input->post('DocDate'));
 ...
 $Detail['Separating'] = $this->input->post('Separating') ;
 $Detail['QtyStroke'] = $this->input->post('QtyStroke') ;
 $Detail['QtyStrokePlan'] = $this->input->post('QtyStrokePlan') ;
 $Detail['Start'] = $this->app_model->tgl_sql($this->input->post('Start')) ;
 $Detail['Finish'] = $this->app_model->tgl_sql($this->input->post('Finish')) ;
 $Detail['Durasi'] = $this->input->post('Durasi') ;
 $Detail['Proses'] = $this->input->post('ProsesProduction') ;
 ...
 $Detail['QtyPlan'] = $this->input->post('QtyPlan') ; 
 $Detail['NG'] = $this->input->post('NG') ;
 $Detail['Yield'] = $this->input->post('Total') ;
 $Detail['Achievement'] = $this->input->post('Achievement') ;
