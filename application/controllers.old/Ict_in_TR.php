<?php
class Ict_in_TR extends CI_Controller{
function __construct(){
 parent::__construct();
 $this->load->model(array('Material_in_TR_model','app_model','Ict_in_TR_model'));
 ...
 elseif(empty($cek)){ redirect('welcome'); }}
    ...
    function index(){
        ...
        $MasterList=$this->Ict_in_TR_model->MasterList();
        ...
        $MListPartner = $this->Ict_in_TR_model->MListPartner();
        ...
        $this->template->display('Ict_in_TR/index',$d);
    }
    
    public function transaction_list()
    {
        $cek=$this->Ict_in_TR_model->transaction_list();
        $data['list']=$cek->result();
        $this->load->view('Ict_in_TR/transaction_list',$data);
    }
    ...
    public function InfoTambahFormTRMatIn(){ 
        ...
        if(empty($id)){
            $DocNumDetail = '001'   ; 
            $DocNum = $this->app_model->DocNumMaterialINICTOther();
        }else{
            $DocNumDetail = $this->app_model->DocNumDetailictINTR($id);
            $id = $this->input->post('kode');  
        }
        //Time SERVER,
        date_default_timezone_set('Asia/Jakarta');		    
 ...
 echo json_encode($data); } }        
    ...
    function transaction_detail(){
        $id = $this->input->post('kode');
        $cek =$this->Ict_in_TR_model->transaction_detail($id);
                
        $data['list']=$cek->result();
        //echo $cek;                                        
        $this->load->view('Ict_in_TR/transaction_detail',$data); 
    }
    ...
    function MasterList(){
        ...
        $this->load->view('Ict_in_TR/master_list',$data); 
     } 
    /**{{}}**/
    function transaction_detail_2(){
        ...
        $cek =$this->Ict_in_TR_model->transaction_detail($id);
        $data['list']=$cek->result();
        $this->load->view('Ict_in_TR/transaction_detail_2',$data); 
    }
     /**{{}}**/ 
    ...
    public function ReadReport()
    {
        ...
        $cek = $this->Ict_in_TR_model->transaction_detail_report($tgl1,$tgl2,$IDCategory,$ItemID,$PartnerID);
        $d['list']=$cek->result();              
        $this->load->view('Ict_in_TR/transaction_detail_report',$d);
    }
    /**{{}}**/
    ...
    public function ExportReport()
    {
        ...
        $d['data']=$cek->result(); 
        $d['num'] = $cek->num_rows();                
        $this->load->view('Ict_in_TR/ExportListReport',$d); 
    }
    /**{{}}**/
    ...
    function MasterList2()
    {
        $DB=$this->Ict_in_TR_model->MasterList();
        $data['MListProduct']=$DB->result();
        $this->load->view('Ict_in_TR/master_list2',$data); 
    }     
    /**{{}}**/         
    ...
    function cekstok()
    {
        ...
        $data['stock'] = $this->app_model->db_get_one("SELECT StockWip FROM M_Product WHERE RegID='".$ItemID."'");
