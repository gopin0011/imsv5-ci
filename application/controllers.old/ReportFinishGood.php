<?php
class ReportFinishGood extends CI_Controller{
function __construct(){
 parent::__construct();
 $this->load->model(array('ReportFinishGood_model','app_model'));
...
function index(){
 ...
 $this->template->display('ReportFinishGood/index',$d); }
    
function MasterList(){
 $id = $this->input->post('kode');    
 $DB = $this->ReportFinishGood_model->MasterList($id);
 $data['MListProduct']=$DB->result();
 $this->load->view('ReportFinishGood/master_list',$data); } 

function MasterList2(){
 $DB = $this->ReportFinishGood_model->MasterList2();
 $data['MListProduct2']=$DB->result();
 $this->load->view('ReportFinishGood/master_list2',$data); }    
...
public function ReadReport(){
 ...
 $DB = $this->ReportFinishGood_model->transaction_detail_report($IDCust,$ItemID);
 $d['list']=$DB->result(); 
 $this->load->view('ReportFinishGood/ViewReport',$d); }
...
$DB = $this->ReportFinishGood_model->transaction_detail_report($IDCust,$ItemID);
 $d['list']=$DB->result(); 
 $d['num'] = $DB->num_rows();
 $this->load->view('ReportFinishGood/ExportListReport',$d); }
...
public function ReadStockCard(){
 ...
 $DB =$this->ReportFinishGood_model->ReadStockCard($tgl1,$tgl2,$ItemID);
 $d['list']=$DB->result();
 $d['num'] = $DB->num_rows();
 $this->load->view('ReportFinishGood/ViewStockCard',$d); }

public function Save(){
 $up['PcsPerday']            = $this->input->post('PcsPerDay');
 $up['StdPack']            = $this->input->post('StdPack');
 $up['StockFG']              = $this->input->post('Qty');
 $up['IsActive']             = $this->input->post('IsActive');
 $id['RegID']               = $this->input->post('ItemID');
 $data = $this->app_model->getSelectedData("M_Product",$id);
 if($data->num_rows()>0){
 $this->app_model->updateData("M_Product",$up,$id);
 echo 'Edit data sukses bro';
 }else{
 echo 'Data Tidak Ada';		
 }  }
