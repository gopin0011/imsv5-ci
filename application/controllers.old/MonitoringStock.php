<?php
class MonitoringStock extends CI_Controller{

function __construct(){
parent::__construct();
$this->load->model(array('MonitoringStock_model','app_model'));
$this->load->library(array('form_validation','template2'));
}
    
function index(){
 $d['title']="Home";
 $text2 = "SELECT * FROM MonitoringRMListCust ORDER BY Code";
 $d['l_MCust'] = $this->app_model->manualQuery($text2);
  $d['counter'] = "32000" ;
 $d['visit_counter'] = "30" ;
$this->template2->display('MonitoringStock/index',$d); }

public function NextID(){
 $id = $this->input->post('id');
 $text = "SELECT * FROM MonitoringRMListCust WHERE RegID='$id'";
...
public function NextIDCust(){
 $id = $this->input->post('id');
 $text = "SELECT * FROM MonitoringRMListCust WHERE NOMOR='$id'";
 $tabel = $this->app_model->manualQuery($text);
 $row = $tabel->num_rows();
 if($row>0){
 foreach($tabel->result() as $t){ 
...
public function ListProduct(){
 ...
 $total_cust = $this->app_model->total_cust_item($id) ;
 $data_danger = $this->app_model->cari_percent_danger($id) ;
 $data_warning = $this->app_model->cari_percent_warning($id) ;
 $data_safe = $this->app_model->cari_percent_safe($id) ;
 $data_warning_up = $this->app_model->cari_percent_warning_up($id) ;
 $data_danger_up = $this->app_model->cari_percent_danger_up($id) ;
 if($total_cust==0){
 ...
 $DB =$this->MonitoringStock_model->MasterList($id);
 $d['list']=$DB->result();
 $d['num'] = $DB->num_rows();
 
 
 $this->load->view('MonitoringStock/master_list',$d);
  }
