...
public function InfoBOM_Head() {
 ...
 $data['IDCust'] = $t->IDCust;
    ...
    public function InfoDataEdit()
	{
			...
			$kode = $this->input->post('kode');
            $text = "SELECT a.* FROM QTD_RawMaterial a WHERE a.DocNumDetail='$kode' " ;
...
date_default_timezone_set('Asia/Jakarta');		    
        ...
        /*{{}}*/
        public function InfoDataEditIct()
	{

			$kode = $this->input->post('kode');
            $text = "SELECT * FROM QTD_Ict WHERE DocNumDetail='$kode' " ;
			$tabel = $this->app_model->manualQuery($text);
			$row = $tabel->num_rows();
			if($row>0){
				foreach($tabel->result() as $t){ 
...
date_default_timezone_set('Asia/Jakarta');	
    ...
    public function InfoTambahFormInBPFG()
	{ 
          $id = $this->input->post('kode');
          if(empty($id))
          {
...
date_default_timezone_set('Asia/Jakarta');	
    ...
    /*{{}}*/        
    public function InfoTambahFormTRIctOut()
	{ 
        $id = $this->input->post('kode');
        if(empty($id)){
            $DocNumDetail	        = '001'   ; 
            $DocNum                 = $this->app_model->DocNumIctOUTRMOther();
        }
        else{
