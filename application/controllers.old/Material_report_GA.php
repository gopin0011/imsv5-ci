<?php
class Material_report_GA extends CI_Controller{
function __construct(){
 parent::__construct();
 $this->load->model(array('Material_report_GA_model','app_model'));
...
function index(){
 $d['title']="Home";
 $text2 = "SELECT * FROM M_Category WHERE GroupBy='GA' ORDER BY id DESC" ;
...
$this->template->display('Material_report_GA/index',$d); }
    
function MasterList(){
$DB = $this->Material_report_GA_model->MasterList();
$data['MListProduct']=$DB->result();
$this->load->view('Material_report_GA/master_list',$data); } 
...
public function ExportReport(){
 ...
 $DB =$this->Material_report_GA_model->transaction_detail_report($IDCategory,$part_no,$spec);
 $d['list']=$DB->result();
 $d['num'] = $DB->num_rows();
 $this->load->view('Material_report_GA/ExportListReport',$d); }
...
$DB =$this->Material_report_GA_model->transaction_detail_report($IDCategory,$part_no,$spec);
$d['list']=$DB->result();
$this->load->view('Material_report_GA/ViewReport',$d);}  
...
public function ReadStockCard(){
 ...
 $DB =$this->Material_report_GA_model->ReadStockCard($tgl1,$tgl2,$ItemID);
 $d['list']=$DB->result();
 $d['num'] = $DB->num_rows();
 $this->load->view('Material_report_GA/ViewStockCard',$d); }
