<?php
class Material_out_GA extends CI_Controller{
function __construct(){
 parent::__construct();
 $this->load->model(array('Material_out_GA_model','app_model'));
 $this->load->library(array('form_validation','template'));
 $cek = $this->session->userdata('TrcGA')=='1';
...
function index(){
 ...
 $MasterList=$this->Material_out_GA_model->MasterList();
 ...
 $MListPartner = $this->Material_out_GA_model->MListPartner();
 $d['MasterPartner'] = $MListPartner->result(); 
 $text3 = "SELECT * FROM M_customer ";
 $d['l_cust'] = $this->app_model->manualQuery($text3);
 $text4 = "SELECT * FROM M_Category WHERE GroupBy='GA' ORDER BY id DESC" ;
 $d['MListCategory'] = $this->app_model->manualQuery($text4);
 $text7 = "SELECT * FROM M_Department ";
 $d['MListDepartment'] = $this->app_model->manualQuery($text7);
...
$this->template->display('Material_out_GA/index',$d);  }
    
function MasterList(){
 $DB=$this->Material_out_GA_model->MasterList();
 $data['MListProduct']=$DB->result();
 $this->load->view('Material_out_GA/master_list',$data); } 
 
function MasterList2(){
 $DB=$this->Material_out_GA_model->MasterList2();
 $data['MListProduct']=$DB->result();
 $this->load->view('Material_out_GA/master_list2',$data); }  
 
function MasterListPic(){
 $DB=$this->Material_out_GA_model->MasterListPic();
 $data['MListProduct']=$DB->result();
 $this->load->view('Material_out_GA/master_list_pic',$data); }   
...
function transaction_list(){
 $cek=$this->Material_out_GA_model->transaction_list();
 $data['list']=$cek->result();
 $this->load->view('Material_out_GA/transaction_list',$data); }
    
public function DetailPrint(){
 $d['judul'] = "Data Material Masuk";
 $id = $this->uri->segment(3);
 $cek =$this->Material_out_GA_model->transaction_detail($id);
 $d['data']=$cek->result();					
 $this->load->view('Material_out_GA/PrintMaterialIn',$d);  }
...
public function PrintList(){
 ...
 $d['data']=$cek->result();
 $d['num'] = $cek->num_rows();					
 $this->load->view('Material_out_GA/ExportList',$d); }
...
public function PrintList2(){
 ...
 $d['IDCust'] = $this->app_model->CariCustName($this->uri->segment(5));}else{
          ...
          $d['ItemID'] = $this->app_model->CariProductName($this->uri->segment(7));           
            ...
            $cek = $this->Material_out_GA_model->transaction_detail_report($DocDateReport_1,$DocDateReport_2,$IDCust,$DocNum,$PartnerID);
            $Stock = $this->Material_out_GA_model->Stock_List();
            $d['data']=$cek->result(); 
            $d['num'] = $cek->num_rows(); 
			$this->load->view('Material_out_GA/PrintList2',$d);
	}
...
function transaction_detail(){
 ...
 $cek =$this->Material_out_GA_model->transaction_detail($id);
 $data['list']=$cek->result();
 $this->load->view('Material_out_GA/transaction_detail',$data); }
    
function transaction_detail_2(){
 $id = $this->input->post('kode');
 $cek =$this->Material_out_GA_model->transaction_detail($id);
 $data['list']=$cek->result();
 $this->load->view('Material_out_GA/transaction_detail_2',$data); }
...
public function ReadReport(){
 ...
 $cek = $this->Material_out_GA_model->transaction_detail_report($tgl1,$tgl2,$IDCategory,$ItemID,$IDDepartment);
 $d['list']=$cek->result();              
 $this->load->view('Material_out_GA/transaction_detail_report',$d);
	}
...
public function ExportReport(){
 ...
 $d['judul'] = "TR Product - OUT";
 $d['filter'] = $this->app_model->tgl_indo($tgl1).' ~ '.$this->app_model->tgl_indo($tgl2);
 if($IDCategory!='semua'){
 $d['IDCategory'] = $this->app_model->CariCategoryName($this->uri->segment(5));}else{
 $d['IDCategory'] = "All Category"; }
 if($IDDepartment!='semua'){
 $d['IDDepartment'] = $this->app_model->CariDepartmentName($this->uri->segment(6));}else{
 $d['IDDepartment'] = "All Department"; }
 $d['ItemID'] = $this->app_model->CariProductName($this->uri->segment(7));       

 $cek = $this->Material_out_GA_model->transaction_detail_report($tgl1,$tgl2,$IDCategory,$ItemID,$IDDepartment);
 $d['data']=$cek->result(); 
 $d['num'] = $cek->num_rows(); 
 $this->load->view('Material_out_GA/ExportListReport',$d); }
...
public function DetailPrint2(){
 ...
 $d['DocNum'] =$id;
 $d['DocDate']	    =''; } 
 $cek =$this->Material_out_GA_model->transaction_detail2($id);
 $d['data']=$cek->result();					
 $this->load->view('Material_out_GA/PrintMaterialIn',$d);   }
...
function Save(){
 ...
 $Detail['Amount'] = $this->input->post('Price') ; 
 $Detail['BalAmount'] = $this->input->post('Price') ; 
 $Detail['IDPic'] = $this->input->post('IDPic') ;     
 $Detail3['BalMat'] = $this->input->post('BalMat') ;
 $Detail3['BalAmount']  = $this->input->post('BalAmount') ; 
 ...
 $this->app_model->updateData("TD_RawMaterial",$Detail3,$IndexDetail4);
...
date_default_timezone_set('Asia/Jakarta');
 ...
 $up2['BalMat'] = $this->input->post('BalMatSourceAF') ;
 $up2['BalAmount'] = $this->input->post('BalAmountSourceAF') ;
