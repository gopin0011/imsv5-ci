    ...
    <script>
        function collapse(obj,no)
        {
            ...
            else
            {
                ...
                $.ajax({
                    type	: 'POST',
                    url		: "<?php echo site_url(); ?>/SuratJalanSbc/GetDetailFakturSJ",
                    ...
                    success	: function(data)
                    {
                        ...
                        $.each($.parseJSON(data).list, function(i,el){
                            //alert(el.DocNum);
                            var string = '<tr id="list'+id+'" class="list'+id+'"><td width="20">'+no+'.'+y+'</td><td>'+el.DocNum+'</td><td>'+el.PartnerCode+'</td><td>'+el.DocDate+'</td><td>'+el.CarNum+'</td><td>'+el.UserName+'</td><td align="right">'+el.Quantity+'</td></tr>';
