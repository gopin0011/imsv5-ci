...
$('#reload').on('click', function() {
 ...
 $.ajax({
 type	: 'POST',
 url		: "<?php echo site_url(); ?>/BPFG/TransactionList",
 data	: "kode="+kode,
 cache	: false,
...
function Detail_data2(){
 ...
 $.ajax({
 type	: 'POST',
 url		: "<?php echo site_url(); ?>/BPFG/DataDetailMatIn2",
 data	: "kode="+kode,
 cache	: false,
 success	: function(data){
...
function AmbilForm(){
 ...
 $.ajax({
 type	: 'POST',
 url		: "<?php echo site_url(); ?>/ref_json/InfoTambahFormInFG",
 data	: "kode="+kode,
 cache	: false,
 dataType : "json",
 success	: function(data){
...
$('#tab_tambah_detail').on('click', function() {
 ...
 Detail_data(); Detail_data2();
   ...
   function AmbilFormDetail(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/ref_json/InfoTambahFormDetailInFG",
			data	: "kode="+kode,
			cache	: false,
			dataType : "json",
			success	: function(data){
...
$("#tab_tambah_detail").keyup(function(){
 Detail_data(); AmbilFormDetail();	 });
	...
	function Detail_data(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/BPFG/DataDetailMatIn",
			data	: "kode="+kode,
			cache	: false,
			success	: function(data){
...
$("#simpan").click(function(){
 ...
 if(Qty<=0){ NotifFail('Qty tidak boleh kurang dari nol'); return false(); }
    ...
    if(FromArea.length==0){  NotifFail('Dari Area harus diisi'); return false(); } 
 ...
 var win = waitingDialog3.show('Processing ...', {dialogSize: 'sm', progressType: 'warning'}); 
  $.ajax({
 type	: 'POST',
 url	: "<?php echo site_url(); ?>/BPFG/simpan",
 data	: string,
 cache	: false,
 success	: function(data){
 setTimeout(function(){
...
$("#Search").click(function(){
 ...
 $.ajax({
 type	: 'POST',
 url		: "<?php echo site_url(); ?>/BPFG/ReadReport",
 data	: string,
 cache	: false,
 success	: function(data){
 setTimeout(function(){
...
$("#Download3").button('reset'); }, 1000);
 ...
 window.open('<?php echo site_url();?>/BPFG/ExportReport/'+string); 
...
$("#PrintList3").button('reset'); }, 1000);
 ...
 window.open('<?php echo site_url();?>/BPFG/PrintList2/'+string , 'myWindow', 
'status = 1, height = 650, width = 1100, resizable = 0, toolbar=no, location=no, status=no&#39, menubar=no, address bar=no' );
        ...
        function BackHome(){
        window.location.replace("<?php echo base_url();?>index.php/BPFG/");
        }
...
$("#HapusDetail").click(function(){
 ...
 $.ajax({
 type	: 'POST',
 url		: "<?php echo site_url(); ?>/BPFG/Hapus_Transaksi",
...
$("#PrintList1").click(function(){
 var kode	= $("#DocNum").val();
 window.open('<?php echo site_url();?>/BPFG/PrintList/'+kode , 'myWindow', 
 'status = 1, height = 650, width = 1100, resizable = 0, toolbar=no, location=no, status=no&#39, menubar=no, address bar=no' );
 return false(); });
...
$("#PrintList2").click(function(){
 ...
 $('#PrintListKecil').click(function(){
    var kode = $("#DocNum2").val();
    window.open('<?php echo site_url();?>/BPFG/PrintListKecil/'+kode , 'myWindow', 'status = 1, height = 650, width = 1100, resizable = 0, toolbar=no, location=no, status=no&#39, menubar=no, address bar=no');
    return false();
 });
...
$("#Download2").click(function(){
 ...
 window.open('<?php echo site_url();?>/BPFG/ExportList/'+kode); 
 return false(); });
...
function MasterList(){
 ...
 $.ajax({
 type	: 'POST',
 url		: "<?php echo site_url(); ?>/BPFG/MasterList",
 data	: "kode="+kode,
 cache	: false,
 success	: function(data){
 setTimeout(function() {
...
function List_Material(){
 ...
 $.ajax({
 type	: 'POST',
 url		: "<?php echo site_url(); ?>/BPFG/DataDetailMatIn3",
 data	: "kode="+kode,
 cache	: false,
 success	: function(data){
 setTimeout(function() {
...
<!--
<div class="form-group">
<label class="col-xs-4 control-label">No. BPFG</label>
<div class="col-xs-8">
<input type="text" id="DocNumExt" name="DocNumExt"  class="form-control">
</div></div>
-->
</div> 
...
<select name="FromArea" id="FromArea" class="form-control">
	<option value="">PILIH</option>
	<?php foreach($area as $t) { ?>
	<option value="<?php echo $t->SysID;?>"><?php echo $t->SysID;?> - <?php echo $t->Location;?></option>
	<?php } ?>  
</select>
...
<button type="button" name="PrintListKecil" id="PrintListKecil" class="btn btn-warning"><i class="glyphicon glyphicon-print"></i> Print Kecil</button>
