...
<script>
    ...
    $(document).ready(function()
    {
        ...
        function MasterList2()
        {
            ...
            $.ajax({
                type	: 'POST',
                url		: "<?php echo site_url(); ?>/SuratJalan/MasterList2",
                data	: "kode="+kode,
                cache	: false,
                success	: function(data)
                {
        ...
        $("#Search").click(function()
        {
            ...
            $.ajax({
                type	 : 'POST',
                url	 : "<?php echo site_url(); ?>/SuratJalan/ReadReport",
                data	 : string,
                cache	: false,
                success	: function(data){
                setTimeout(function()
        ...
        $('#PrintList3').click(function(){
            ...
            window.open('<?php echo site_url();?>/SuratJalan/PrintListReport/'+string , 'myWindow', 
            'status = 1, height = 650, width = 1100, resizable = 0, toolbar=no, location=no, status=no&#39, menubar=no, address bar=no' );
            return false();
        });
...
<button type="button" name="tolak" id="tolak" class="btn btn-warning"><i class="glyphicon glyphicon-remove" id="b_tolak"></i> Tolak</button>
</div></div>
</div>
...
<script>
    function notif_confirm()
    {
        $.ajax({
            type	: 'POST',
            url		: "<?php echo site_url(); ?>/SuratJalan/GetNotif",
            data	: "kode=",
            cache	: false,
            success	: function(data)
            {
                var d = JSON.parse(data);
                $('#notif_confirm').html(d.Waiting);
            } 
        });
    ...
    function proses_confirm(obj,status)
    {
        ...
        $.ajax({
            type	: 'POST',
            url		: "<?php echo site_url(); ?>/SuratJalan/update_confirm/",
...
<script>
    ...
    */
    function cari_docnum()
    {
        ...
        $.ajax({
                type	: 'POST',
                url		: "<?php echo site_url(); ?>/SuratJalan/GetDocNumReport",
                data	: "DocNum="+val,
                cache	: false,
                success	: function(data)
                {
...
<div class="modal fade" id="myModal_confirm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-options="closed:true">
<div class="modal-dialog modal-lg"> <div class="modal-content"> <div class="modal-header">
<button type="button" class="close" data-dismiss="modal" aria-hidden="true">Cl&times;se</button>
