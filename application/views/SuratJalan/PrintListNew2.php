...
<?php
    ...
    if($IsConfirm == 2)
    {
        echo '<div class="row"><div class="form-group"><label class="control-label col-sm-2"><font style="color:red">Keterangan:</font></label><div class="col-sm-10"><textarea id="Remark" class="form-control" name="Remark" disabled="disabled">'.$KetStatus.'</textarea></div></div></div>';
    }
...
<script>
    $(document).ready(function(){
        if($('#IsConfirm').val()=='1')
        {
            ...
            //$('#tolak').attr({'class':'btn btn-warning','onclick':'proses_confirm(this,2);'});
            $('#tolak').attr({'class':'btn btn-warning'});
            var attr = $('#tolak').attr('onclick');
            if (typeof attr !== typeof undefined && attr !== false) {
                $('#tolak').removeAttr('onclick');
            }
        }
        else if($('#IsConfirm').val()=='2')
        {
            $('#confirm').attr({'class':'btn btn-success'});
            $('#tolak').attr({'class':'btn btn-default active','onclick':'proses_confirm(this,0);'});
            var attr2 = $('#confirm').attr('onclick');
        ...
        $('#tolak').click(function(){
            if($('#IsConfirm').val()!='2')
            {
                $.ajax({
                    type	: 'POST',
                    url		: "<?php echo site_url(); ?>/SuratJalan/GetTemplatesConfirm/",
                    data	: "type=2&RegID="+$('#RegIDConfirm').val(),
                    cache	: false,
                    success	: function(data)
                    {
                        var d = JSON.parse(data);
                        $('#title_confirm').html('Tolak Surat Jalan ? ' + d['surjal']);
                        $('#divConfirm').html(d['templates']);
        ...
        $('#confirm').click(function(){
            if($('#IsConfirm').val()!='1')
            {
                $.ajax({
                    type	: 'POST',
                    url		: "<?php echo site_url(); ?>/SuratJalan/GetTemplatesConfirm/",
                    data	: "type=1&RegID="+$('#RegIDConfirm').val(),
                    cache	: false,
                    success	: function(data)
                    {
                        var d = JSON.parse(data);
                        $('#title_confirm').html('Confirm ? ' + d['surjal']);
                        $('#divConfirm').html(d['templates']);
                        $("#myModal_confirm").modal('show');
                    } 
                });
