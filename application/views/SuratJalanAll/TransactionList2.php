...
<a style="CURSOR: pointer;" onclick="print_this('<?php echo $row->RegID;?>');" href="#tab_content5" data-toggle="tab" aria-expanded="false">
...
{
    //window.open('<?php echo site_url();?>/SuratJalanAll/PrintList2/'+RegID,'',' scrollbars=yes,menubar=no,width=1100,height=600, resizable=yes,toolbar=no,location=no,status=no');
    $.ajax({
        type	: 'POST',
        url		: "<?php echo site_url(); ?>/SuratJalanAll/PrintList2/",
        data	: "RegID="+RegID,
        cache	: false,
        success	: function(data)
        {
            //$('#header_data2').attr('style','padding: 0 100 0 100');
            $("#header_data2").html(data); 
        } 
    });
}
</script>
