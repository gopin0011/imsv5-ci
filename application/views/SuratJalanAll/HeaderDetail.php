...
<div class="row">
    <div class="col-md-12">
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <input id="DocNumDetail" type="text" name="Rev" class="form-control" placeholder="" required="required" data-error="" readonly="readonly" value="<?php echo $header->DocNumDetail;?>">
            </div>
        </div>
    </div>
</div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <select name="PartnerID" id="PartnerID" class="form-control">
                	<option value="<?php echo $header->PartnerID;?>"><?php echo $header->PartnerName;?></option>
                </select>
            </div>
            <div class="col-sm-5">
                <input id="PartnerName" type="text" name="PartnerName" class="form-control" placeholder="" required="required" data-error="" readonly="readonly" value="<?php echo $header->PartnerName.' - '.$header->PartnerCode;?>">
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function(){
        $('#formSJ input,#formSJ textarea').attr('readonly', 'readonly');
        $('#formSJ select').attr('disabled', 'true');
        $('#PrintList2').attr('data-ref',$('#RegID').val());
        $('#ExportList').attr('data-ref',$('#RegID').val());
    });
</script>
