...
<?php  $cek = $this->Role_Model->MUserTRUp() ; if(!empty($cek)){ ?>
...
<?php  $cek = $this->Role_Model->MUserTRView() ; if(!empty($cek)){ ?>
...
<input type="text" id="RegID" name="RegID"  class="form-control" readonly="true">
 ...
 <?php  $cek = $this->Role_Model->MUserTRDel() ; if(!empty($cek)){ ?>
 <button type="button" name="Hapus_User" id="Hapus_User" class="btn btn-danger"><i class="glyphicon glyphicon-trash"></i>&nbsp; Hapus</button>  <?php } ?>      
 </div></div>
</div></div></div>
</div></div></div>
</div>
