...
<?php foreach($list as $r =>$v)
{
    ...
    <?php
        ...
        {
            echo'<tr id="'.$vv->RegID.'">
                     <th><input type="hidden" name="qtystore_'.$vv->RegID.'" id="qtystore_'.$vv->RegID.'" value="'.$vv->QtyMat.'"><input type="hidden" name="ketstore_'.$vv->RegID.'" id="ketstore_'.$vv->RegID.'" value="'.$vv->ket.'">'.$no.'</th>
                     <th>'.$vv->Raw_DocNum.'</th>
                     <th>'.$vv->PartNo.'</th>
                     <th>'.$vv->PartName.'</th>
                     <th>'.$vv->Code.'</th>
                     ...
                     <th><a id="id_'.$vv->RegID.'" onclick="PilihEdit(\''.$vv->RegID.'\',\'\',\'\')" data-qty="'.$vv->QtyMat.'" data-ket="'.$vv->ket.'" href="javascript:void(0);" data-toggle="tab" aria-expanded="false">
...
<script>
    ...
    function PilihEdit(RegID,QtyMat,Ket)
    {
		...
		QtyMat = $('#qtystore_'+RegID).val();
		Ket = $('#ketstore_'+RegID).val();
        ...
        setTimeout(function()
        {
            $.ajax({
                ...
                success	: function(data)
                {
                    ...
                    $('#Qty_'+RegID).html(QtyMat);
                    $('#Ket_'+RegID).html(Ket);
					$('#qtystore_'+RegID).val(QtyMat);
					$('#ketstore_'+RegID).val(Ket);
                    $('#id_'+RegID).attr({'data-qty':QtyMat,'data-ket':Ket});
                    //$('#id_'+RegID).attr('data-ket',Ket);
                } 
            });
        },800); 
    }
