...
<?php $no=0; foreach($list as $row): 
 ...
 $IN  = $this->app_model->QtyGA_IN($row->ItemID,$tgl_awal,$tgl_akhir);
 $OUT  = $this->app_model->QtyGA_OUT($row->ItemID,$tgl_awal,$tgl_akhir);
 $IN_F  = $this->app_model->QtyGA_IN($row->ItemID,$tgl_awal,$tgl_akhir);
 $OUT_F  = $this->app_model->QtyGA_OUT($row->ItemID,$tgl_awal,$tgl_akhir);
 if($IN_F==0){$IN_R='-';}else{$IN_R= number_format($IN_F);}
 if($OUT_F==0){$OUT_R='-';}else{$OUT_R= number_format($OUT_F);}
 $IN_2  = $this->app_model->QtyGA_IN($row->ItemID,$tgl_akhir_2,$tgl_sekarang2);
 $OUT_2  = $this->app_model->QtyGA_OUT($row->ItemID,$tgl_akhir_2,$tgl_sekarang2);
 $IN_Amount  = $this->app_model->QtyAmountGA_IN($row->ItemID,$tgl_akhir_2,$tgl_sekarang2);
 $OUT_Amount = $this->app_model->QtyAmountGA_OUT($row->ItemID,$tgl_akhir_2,$tgl_sekarang2);
