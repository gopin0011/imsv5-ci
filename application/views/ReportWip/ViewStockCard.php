...
foreach($list as $row){
 ...
 $cust = $row->Code;
 $input = $this->app_model->CariJmlinput_WIP($row->ItemID,$tgl_awal,$tgl_akhir);
 $input2 = $this->app_model->WIPStock($row->ItemID);
 $input3 = $this->app_model->CariJmlinput_WIP($row->ItemID,$tgl_awal,$tgl_akhir2);
 ...
 if(strpos($Reg1, 'WIP')!== FALSE){$Reg='IN';}
 if(strpos($Reg1, 'OUT')!== FALSE){$Reg='OUT';}
 $IN1 = $row->INMat;
