    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            if($(input).attr('id')=='imgInp')
                ...
                $('#img_preview4').attr({'src' : e.target.result});
        }
...
$("#imgInp, #imgInp4").change(function(){
    readURL(this);
});
*/
...
$('#BAddBack, #BAddBackab').click(function(){
	var i = 500;
    setTimeout(function(){
		$('#formaddchild').hide();
        //$('#formaddchild2').hide();
...
function addSavePart(type,elem)
{
    var id = '#'+$(elem).attr('id');
    var max_file_size 			= 2048576; //allowed file size. (1 MB = 1048576)
    var allowed_file_types 		= ['image/png', 'image/gif', 'image/jpeg', 'image/pjpeg']; //allowed file types
    var total_files_allowed 	= 1;  
    var form_data = new FormData(elem); //Creates new FormData object
    form_data.append('file',$(id+' input[name=file]')[0].files[0]);
    //var PartNo2 = $(id+' input[name=PartNo2]').val();
    if($(id+' input[name=PartNo2]').val().length==0){
		NotifFail('Part No LH field is required !!!');
        return false();
    }
    
    if($(id+' select[name=LevelPart]').val().length==0){
		NotifFail('Level Part Belum di Pilih !!!');
        return false();
    } 
    
    if($(id+' input[name=PartName2]').val().length==0){
		NotifFail('Part Name field is required !!!');
        return false();
    } 
    
    if($(id+' select[name=PartTypeID2]').val().length==0){
		NotifFail('Part Type field is required !!!');
        return false();
    }
    
    if($(id+' select[name=IsRHLH]').val() == 1 && $(id+' input[name=PartNo3]').val().length==0)
    {
		NotifFail('If IS RH LH.. Part No RH, field is required !!!');
        return false();
    }
    
    if($(id+' select[name=PartTypeID2]').val().indexOf('RM')>=0)
    {
        //PartRM = null;
        //PartNoMap = null;
    }
	...
	$.ajax({
        ...
        NotifSuccsess(res.pesan);
        setTimeout(function(){
            $(id+' input[name=ItemIDSys2]').val(res.ItemID);
            if(type==0)
                editthisBOM2(res.ItemID);
            else
            {           
                if(res.pesan == 'Updating Success !!!') {
                    
                }
                else setNewChildMap(res.ItemID,res.PartNo2,res.PartName2);
            }
...
function addSavePartChild(type)
{
    var kode = $("#SysIDDetail").val();
    ...
    var OP5 = $("#OP54").val(); var OP5M = $("#OP5M4").val();
    var OP10 = $("#OP104").val(); var OP10M = $("#OP10M4").val();
    var OP40 = $("#OP404").val(); var OP40M = $("#OP40M4").val();
    var OP20 = $("#OP204").val(); var OP20M = $("#OP20M4").val();
    var OP50 = $("#OP504").val(); var OP50M = $("#OP50M4").val();
    var OP30 = $("#OP304").val(); var OP30M = $("#OP30M4").val();
    var OP60 = $("#OP604").val(); var OP60M = $("#OP60M4").val();
    var OP70 = $("#OP704").val(); var OP70M = $("#OP70M4").val();
    var ProcessAssy = $("#ProcessAssy4").val(); var ProcessAssyM = $("#ProcessAssyM4").val();
    
    //var PartRM = $('#PartRM').val();
    //var PartNoMap = $('#PartNoMap').val();
    
    if(PartNo2.length==0){
		NotifFail('Part No field is required !!!');
        return false();
    } 
    ...
    {
		NotifFail('Part No 2, field is required !!!');
        return false();
    }
    
    if(PartTypeID2.indexOf('RM')>=0)
    {
        //PartRM = null;
        //PartNoMap = null;
    }
...
function AddnewPartChild(input)
{
    $.get( "<?php echo site_url();?>/MasterBom/getChildTemplates", function( data ) {
      $( "#formaddchild").hide();
      //$( "#formaddchild2Isi" ).html( JSON.parse(data).templates );
      $('#ItemIDSys2B').val(input.ItemID);
      $('#ItemID2B').val(input.ItemID);
      $('#LinkIDB').val(input.LinkID);
      $('#PartNo2B').val(input.PartNo2);
      $('#PartNo3B').val(input.PartNo3);
      $('#PartName2B').val(input.PartName2);
      $('#PackingTypeDetailB').val(input.PackingTypeDetail);
      $('#StdPackDetailB').val(input.StdPackDetail);
      $('#LevelPartB').val(input.LevelPart);
      $('#PartTypeID2B').val(input.PartTypeID2);
      $('#QtyCarB').val(input.QtyCar);
      $('#SupplierIDB').val(input.SupplierID);
      $('#IsRHLHB').val(input.IsRHLH);
      //$( "#formaddchild2" ).show();
    });
}


//simpan dan menambah part child
$("#simpan3").on("click",function(){
   addSavePart(1); 
});

$('#simpan2bbbb').click(function(){
    addSavePart(0);
});

$("#formSubBom").on("submit",function(){
    addSavePart(0,this);
    return false;
});

$("#formSubRM").on("submit",function(){
    addSavePart(1,this);
    return false;
});
...
$("#PartTypeID4").on("change",function(){
    set_hide_form_child($(this));
});

function set_hide_form_child(ele)
{
    if($(ele).val().indexOf("PC")>=0)
    {
        $("#accordionNewChild1").hide();
        $("#accordionNewChild2").show();
    } 
    else if($(ele).val().indexOf("RM")>=0)
    {
        $("#accordionNewChild1").show();
        $("#accordionNewChild2").hide();
    }
    else
    {
        $("#accordionNewChild1").hide();
...
$('#PartNoMap').on('focus',function(){
    $.ajax({
     type	: 'POST',
     url	: "<?php echo site_url();?>/MasterBom/search",
     data	: "query=&ItemParent=&ItemID="+$(this).val(),
     cache	: false,
     success	: function(data){
        document.getElementById("formSubRM").reset();
        set_hide_form_child($("#PartTypeID4"));
        var d = JSON.parse(data);
        setTimeout(function(){
            $('#modalchild').modal('show');
            $('#isi_modalchild').html(d.templates);
        },200) 	
     }
    });
});
...
$("#Hapus").click(function(){
 ...
 var LevelDetailDelete2	    = $("#LevelDetailDelete2").val();
 $("#myModal3434").modal('hide');
 $.ajax({
 type	: 'POST',
 url	: "<?php echo site_url(); ?>/MasterBom/Hapus_Detail",
 data	: "LevelDetailDelete2="+LevelDetailDelete2+"&DocNumDetailDelete2="+DocNumDetailDelete2,
 cache	: false,
 success	: function(data){
    var d = JSON.parse(data);
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div id="formaddchild" style="display: none;">
                                <form class="form-horizontal" id="formSubBom" enctype="multipart/form-data">
                                <!--<input type="hidden" name="SysIDDetail2a" id="SysIDDetail2a" value="" />-->
                                <input type="hidden" id="type" name="type" class="form-control" value="0" readonly="true">
                                ...
                                <label class="col-sm-4 control-label">Part Child</label>
                                <div class="col-lg-8">
                                    <input type="text" class="form-control input-sm" id="PartNoMap" name="PartNoMap" readonly="readonly" />
                                </div>
                                </div> 
                                <div class="form-group form-group-sm">
                                ...
                                <div class="col-lg-8">
                                    ...
                                    <img src="<?php echo base_url();?>uploads/null" id="img_preview" width="100%" height="200px" class="thumbnail" />
                                </div>  
                                </div>
                                ...
                                <div class="btn-group" data-toggle="btn-toggle">
                                <button class="btn btn-warning" type="button" name="simpan" onclick="$('#formSubBom').submit();">
...
<br /><br /><br />
    ...
    <input type="hidden" id="LevelDetailDelete2" name="LevelDetailDelete2" ></div>
    </form>
...
<div class="modal fade" id="modalchild" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-options="closed:true">
...
<h4 class="modal-title">Info</h4></div><div class="modal-body"><div>
    <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
              <li class="active"><a href="#tab_1v" data-toggle="tab">Child</a></li>
              <li><a href="#tab_2v" data-toggle="tab">New</a></li>
            </ul>
            <div class="tab-content">
              <div class="tab-pane active" id="tab_1v">
                <div id="isi_modalchild"></div>
                
                <div class="panel-footer">
                    <button type="button" data-dismiss="modal" aria-hidden="true" class="btn btn-warning"><i class="glyphicon glyphicon-share"></i> Batal</button>
                    <button type="button" name="baddchild" id="baddchild" class="btn btn-success"><i class="fa fa-plus"></i> Ok</button>
                </div>
              </div>
              <!-- /.tab-pane -->
              <div class="tab-pane" id="tab_2v">
                <form class="form-horizontal" id="formSubRM">
                    <!--<input type="hidden" name="SysIDDetail2a" id="SysIDDetail2a" value="" />-->
                    <input type="hidden" id="type5" name="type" class="form-control" value="1" readonly="true">
                    <input type="hidden" id="PartID5" name="PartID"  class="form-control" readonly="true">
                    <input type="hidden" id="LinkID5" name="LinkID"  class="form-control" readonly="true">
                    <input type="hidden" id="ItemID25" name="ItemID2"  class="form-control" readonly="true">
                    <input type="hidden" id="ItemNoDetail5" name="ItemNoDetail"  class="form-control" readonly="true">
                    <input type="hidden" min="1" max="20" id="ItemNoDetailSub5" name="ItemNoDetailSub"  class="form-control">
                    <div class="panel-group" id="accordionZB">
                    <div class="panel panel-success">
                    <div class="panel-heading">
                    <h4 class="panel-title">
                    <a data-toggle="collapse" data-parent="#accordionZB" href="#product4"><span class="glyphicon ">
                    </span> Data Product &nbsp; 
                    <span style="float: right;"><i class="fa fa-bars"></i></span></a></h4></div>
                    ...
                    <label class="col-lg-4 control-label">Item ID</label>
                    <div class="col-lg-8">
                    <input type="text" id="ItemIDSys4" name="ItemIDSys2"  class="form-control" readonly="">
                    </div></div>
                    <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Part No LH</label>
                    <div class="col-lg-8">
                    <input type="text" id="PartNo4" name="PartNo2"  class="form-control" >
                    </div></div>
                    <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Part No RH</label>
                    <div class="col-lg-8">
                    <input type="text" id="PartNo4b" name="PartNo3"  class="form-control">
                    </div></div>
                     <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Part Name</label>
                    <div class="col-lg-8">
                    <input type="text" id="PartName4" name="PartName2"  class="form-control">
                    </div>
                    </div> 
                    ...
                    <label class="col-lg-4 control-label">Packing Type</label>
                    <div class="col-lg-8">
                    <input type="text" id="PackingTypeDetail4" name="PackingTypeDetail"  class="form-control">
                    </div></div> 
                    
                     <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Standart Packing</label>
                    <div class="col-lg-8">
                    <input type="text" id="StdPackDetail4" name="StdPackDetail"  class="form-control">
                    </div></div>
                      ...
                      <label class="col-lg-4 control-label">Part Level</label>
                      <div class="col-lg-8">
                      <select name="LevelPart" id="LevelPart4" class="form-control">
                      <option value=""></option>
                      <option value="2">2</option>
                      <option value="3">3</option>
                      <option value="4">4</option>      
                      ...
                      <label class="col-lg-4 control-label">Part Type</label>
                      <div class="col-lg-8">
                      <select name="PartTypeID2" id="PartTypeID4" class="form-control">
                      <option value=""></option>
                      <?php foreach($PartTypeNew->result() as $t){ ?>                      
                      <option value="<?php echo $t->SysID;?>"><?php echo $t->PartType;?></option> <?php  } ?> 
                      </select> </div> </div>
                    ...
                    <label class="col-lg-4 control-label">Qty / Car</label>
                    <div class="col-lg-8">
                    <input type="text" id="QtyCar4" name="QtyCar"  class="form-control">
                    </div></div> 
                    ...
                    <label class="col-sm-4 control-label">Supplier</label>
                    <div class="col-sm-8">
                    <select id="SupplierID4" name="SupplierID" class="form-control" style="width: 100%;">
                    <?php if(empty($PartnerID)){ ?>
                    <option value="0">Select</option>
                    ...
                    <label class="col-sm-4 control-label">Is RH LH</label>
                    <div class="col-sm-8">
                    <select name="IsRHLH" id="IsRHLH4" class="form-control">
                      <option value="0">No</option>
                      <option value="1">Yes</option>
                    </select>
                    </div></div>
                    ...
                    <label class="col-sm-4 control-label">Images</label>
                    <div class="col-lg-8">
                        <input type="file" id="imgInp4" name="file" />
                    </div>
                    </div>
                    </div></div></div></div></div></div>
                    
                    <div class="panel-group" id="accordionNewChild1">
                    <div class="panel panel-default">
                    <div class="panel-heading">
                    <h4 class="panel-title">
                    <a data-toggle="collapse" data-parent="#accordionNewChild1" href="#MaterialSpecCollapse4"><span class="glyphicon ">
                    </span> Material Spec &nbsp; 
                    <span style="float: right;"><i class="fa fa-bars"></i></span></a></h5></div>
                    ...
                    <label class="col-lg-4 control-label">Spec Order</label>
                    <div class="col-lg-4">
                    <input type="text" id="SpecOrder41" name="SpecOrder1"  class="form-control">
                    </div>
                    <div class="col-lg-4">
                    <input type="text" id="SpecOrder42" name="SpecOrder2"  class="form-control">
                    </div></div>
                     ...
                     <div class="col-lg-8">
                      ...
                      <select name="MaterialType" id="MaterialType4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                    <option value="0">Type</option>
                    <?php foreach($MaterialTypeList->result() as $t){ if($MaterialType==$t->MaterialName){ ?>
                    <option style="50px" value="<?php echo $t->RegID;?>" selected="selected"><?php echo $t->MaterialName;?></option>
                    <?php }else { ?>
                    ...
                    <label class="col-lg-4 control-label">Spec</label>
                    <div class="col-lg-8">
                    <input type="text" id="Spec4" name="Spec"  class="form-control">
                    </div></div>
                    
                    <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Size</label>
                    <div class="col-lg-2">
                    <input type="text" id="Thick4" name="Thick" value="0" class="form-control" placeholder="T">
                    </div>
                    <label class="col-lg-1 control-label">X</label>
                    <div class="col-lg-2">
                    <input type="text" id="Width4" name="Width" value="0" class="form-control" placeholder="W">
                    </div>
                    <label class="col-lg-1 control-label">X</label>
                    <div class="col-lg-2">
                    <input type="text" id="Length4" name="Length" value="0" class="form-control" placeholder="L">
                    </div></div>
                    
                    <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Qty Part</label>
                    <div class="col-lg-8">
                    <input type="text" id="QtyPart4" name="QtyPart"  class="form-control" value="1">
                    </div></div>
                    
                    <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Ratio</label>
                    <div class="col-lg-8">
                    <input type="text" id="Ratio4" name="Ratio"  class="form-control" value="1">
                    </div></div>
                    ...
                    <label class="col-lg-4 control-label">Pcs / Sheet</label>
                    <div class="col-lg-8">
                    <input type="text" id="PcsPerSheet4" name="PcsPerSheet" value="0" class="form-control">
                    </div></div>
                    
                    <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Kg / Sheet</label>
                    <div class="col-lg-8">
                    <input type="text" id="KgPerSheet4" name="KgPerSheet" value="0" class="form-control">
                    </div></div>
                    
                    <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                    <label class="col-lg-4 control-label">Part Weight</label>
                    <div class="col-lg-8">
                    <input type="text" id="PartWeight4" name="PartWeight" value="0" class="form-control">
                    </div></div>
                    
                      <div class="form-group form-group-sm" style="margin-top: -0.5em;">
                      <label class="col-sm-4 control-label">Is Common</label>
                      <div class="col-sm-8">
                      <select name="IsCommon" id="IsCommon4" class="form-control">
                     <option value="0">False</option>
                     <option value="1">True</option>
                      </select> </div> </div>
                            ...
                            <div class="panel-group" id="accordionNewChild2">
                                <div class="panel panel-default">
                                <div class="panel-heading">
                                <h4 class="panel-title">
                                <a data-toggle="collapse" data-parent="#accordionNewChild2" href="#ProsesOPXZ"><span class="glyphicon " id="timbul2">
                                </span> Proccess & Home Line &nbsp; 
                                <span style="float: right;"><i class="fa fa-bars"></i></span></a></h5></div>
                                
                                <div id="ProsesOPXZ" class="collapse in">
                                ...
                                <label class="col-lg-4 control-label">OP5</label>
                                <div class="col-lg-4">
                                <input type="text" id="OP54" name="OP5"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                
                                <select name="OP5M" id="OP5M4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                <option value="0" >Machine</option>
                                </select>
                                </div></div>
                                ...
                                <label class="col-lg-4 control-label">OP10</label>
                                <div class="col-lg-4">
                                <input type="text" id="OP104" name="OP104"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                <select name="OP10M" id="OP10M4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                <option value="0">Machine</option>
                                </select> </div></div>
                                ...
                                <label class="col-lg-4 control-label">OP20</label>
                                <div class="col-lg-4">
                                <input type="text" id="OP204" name="OP20"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                <select name="OP20M" id="OP20M4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                <option value="0">Machine</option>
                                </select>  </div></div>
                                ...
                                <label class="col-lg-4 control-label">OP30</label>
                                <div class="col-lg-4">
                                <input type="text" id="OP304" name="OP30"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                <select name="OP30M" id="OP30M4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                <option value="0">Machine</option>
                                </select> </div></div>
                                ...
                                <label class="col-lg-4 control-label">OP40</label>
                                <div class="col-lg-4">
                                <input type="text" id="OP404" name="OP40"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                <select name="OP40M" id="OP40M4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                <option value="0">Machine</option>
                                </select> </div></div>
                                ...
                                <label class="col-lg-4 control-label">OP50</label>
                                <div class="col-lg-4">
                                <input type="text" id="OP504" name="OP50"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                <select name="OP50M" id="OP50M4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                <option value="0">Machine</option>
                                </select> </div></div>
                                ...
                                <label class="col-lg-4 control-label">OP60</label>
                                <div class="col-lg-4">
                                <input type="text" id="OP604" name="OP60"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                <select name="OP60M" id="OP60M4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                ...
                                <label class="col-lg-4 control-label">OP70</label>
                                <div class="col-lg-4">
                                <input type="text" id="OP704" name="OP70"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                <select name="OP70M" id="OP70M4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                ...
                                <label class="col-lg-4 control-label">Process Assy</label>
                                <div class="col-lg-4">
                                <input type="text" id="ProcessAssy4" name="ProcessAssy"  class="form-control">
                                </div>
                                <div class="col-lg-4">
                                
                                <select name="ProcessAssyM" id="ProcessAssyM4" class="form-control" onmousedown="if(this.options.length>5){this.size=6;}"  onchange='this.size=0;' onblur="this.size=0;">
                                ...
                                <label class="col-lg-4 control-label">No.</label>
                                <div class="col-lg-8">
                                <input type="number" min="2" max="20" id="NoUrut4" name="NoUrut"  class="form-control">
                                </div></div>
                    ...
                    <div class="panel-footer">
                        <button type="button" data-dismiss="modal" aria-hidden="true" class="btn btn-warning"><i class="glyphicon glyphicon-share"></i> Batal</button>
                        <button type="button" name="bSavechild" id="bSavechild" class="btn btn-success" onclick="$('#formSubRM input[name=LinkID]').val($('#formSubBom input[name=LinkID]').val()); $('#formSubRM').submit();"><i class="fa fa-plus"></i> Save</button>
                        <button type="button" name="bClearchild" id="bClearchild" class="btn btn-danger" onclick="document.getElementById('formSubRM').reset();"><i class="fa fa-refresh"></i> Clear</button>
                    </div>
              </div>
              <!-- /.tab-pane -->              
            </div>
