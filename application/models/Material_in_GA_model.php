<?php class Material_in_GA_model extends CI_Model{
...
$limit= 50; $offset = 0;
return $this->db->query("SELECT * FROM QTH_RawMaterial WHERE IDTrcType=130 ORDER BY RegID DESC OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY");
}
...
function transaction_detail_report($tgl1,$tgl2,$IDCategory,$ItemID,$PartnerID){
 $where = "WHERE DocDate BETWEEN '$tgl1' AND '$tgl2' AND IDTrcType='130'" ; 
 if(empty($ItemID)){
 if($IDCategory!='semua'){
 if($PartnerID!='ALL'){
