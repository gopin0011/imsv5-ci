 ...
 **/
 ...
 function updateData_06042017($table,$data,$field_key){$this->db->update($table,$data,$field_key); }
 ...
 function insertData_17042017($table,$data){ $this->db->insert($table,$data); }
 function manualQuery($q) { return $this->db->query($q); }
 
      
public function MenuActive_button($RegID){
 $this->db->where("SysID",$RegID);
 return $this->db->get("LastAct"); }
...
public function CariLastAct($id){
 $t = "SELECT * FROM LastAct WHERE SysID='$id'";
 $d = $this->app_model->manualQuery($t);
 $r = $d->num_rows();
 if($r>0){
 foreach($d->result() as $h){
...
public function CariPartnerByDoc($id){
 $t = "SELECT * FROM TH_RawMaterial WHERE DocNum='$id'";
 $d = $this->app_model->manualQuery($t);
 $r = $d->num_rows();
 if($r>0){
 foreach($d->result() as $h){
 ...
 return $hasil; }
  ...
  public function CariTotalACC($bln,$thn){
    ...
    public function total_cust_item_FG($id){
		$t = "SELECT IDCust FROM MonitoringFG WHERE IDCust='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
      ...
      public function cari_percent_danger_FG($id){
		$t = "SELECT hari FROM MonitoringFG WHERE (hari < 1) AND IDCust='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function cari_percent_warning_FG($id){
		$t = "SELECT hari FROM MonitoringFG WHERE (hari BETWEEN 1 AND 1.999) AND IDCust='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function cari_percent_safe_FG($id){
        
		$t = "SELECT hari FROM MonitoringFG WHERE (hari BETWEEN 2 AND 3) AND IDCust='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function cari_percent_warning_up_FG($id){
		$t = "SELECT hari FROM MonitoringFG WHERE (hari BETWEEN 3.01 AND 4.999) AND IDCust='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
    ...
    public function cari_percent_danger_up_FG($id){
		$t = "SELECT hari FROM MonitoringFG WHERE hari > 5 AND IDCust='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
...
public function CariUsername($id){
		$t = "SELECT * FROM M_UserG5 WHERE ActivationID='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			foreach($d->result() as $h){
 ...
 public function CariActivation($id){
		$t = "SELECT * FROM M_UserG5 WHERE ActivationID='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
    ...
    public function Nama_lengkap($id){
		$t = "SELECT * FROM M_UserG5 WHERE UserName='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			foreach($d->result() as $h){
    	...
    	public function ItemMMaterial($id2){
		$t = "SELECT RegID FROM Q01_MProduct WHERE IsMaterial=1 AND IDCust='$id2'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
...
public function DocNumDetailMaterialIN_New($id){
 $text = "SELECT max(DocNumDetail) as last FROM TD_Trace WHERE DocNumDetail LIKE '%$id%' " ;
 $data = $this->app_model->manualQuery($text);
 if($data->num_rows() > 0 ){
 foreach($data->result() as $t){
 $lastNoIN=$t->last;
...
public function DocNumDetailMaterialOUT_New($id){
 $text = "SELECT max(DocNumDetail) as last FROM TD_Trace WHERE DocNumDetail LIKE '%$id%' " ;
 $data = $this->app_model->manualQuery($text);
 if($data->num_rows() > 0 ){
 foreach($data->result() as $t){
...
public function DocNumDetailMaterialRet_New($id){
 ...
 $nextNoTransaksi=sprintf('%03s',$nextNoUrut);
 $hasil = $nextNoTransaksi; }
 }else{ $hasil = $nextNoTransaksi ; }
 return $hasil; }

    public function RM_Stock_01($kode){
 ...
 public function CariJmlinput_ks1_New($kode,$tgl_awal,$tgl_akhir){
		...
		return $hasil;
	     ...
	     public function QtyRM_IN_New($kode,$tgl_awal,$tgl_akhir){
		$t = "SELECT ItemID,sum(Qty_1) as jml 
			FROM QTD_Trace  
			WHERE ItemID='$kode' AND (DocDate BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."') AND TrcType=1000
			GROUP BY ItemID";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
	    ...
	    public function QtyRM_OUT_New($kode,$tgl_awal,$tgl_akhir){
		$t = "SELECT ItemID,sum(Qty_1) as jml 
			FROM QTD_Trace  
			WHERE ItemID='$kode' AND (DocDate BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."') AND TrcType=2000
			GROUP BY ItemID";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
      ...
      public function QtyRM_RET_New($kode,$tgl_awal,$tgl_akhir){
		$t = "SELECT ItemID,sum(Qty_1) as jml 
			FROM QTD_Trace  
			WHERE ItemID='$kode' AND (DocDate BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."') AND TrcType=3000
			GROUP BY ItemID";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
...
$CategoryID = "" ;
   ...
   $DeptID = "" ;
   if($id2!='All'){$DeptID .= "AND DeptID='$id2'" ; }else{$DeptID .= "" ; }
        $t = "SELECT RegID FROM M_UserG5 WHERE IsActive='1' $DeptID ";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
 ...
 $t = "SELECT count(SysID) as jml FROM QMaster_Asset $where ";
   ...
   if($id2!='All'){$DeptID .= "AND id_dept='$id2'" ; }else{$DeptID .= "" ; }
		$t = "SELECT RegID FROM M_UserG5 WHERE IsActive='$id' AND IsDelete='X' $DeptID ";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			$hasil = $r;
...
date_default_timezone_set('Asia/Jakarta');
	...
	//ict tambah docnumber
    public function DocNumMaterialINICTOther(){
        //Time SERVER,
		date_default_timezone_set('Asia/Jakarta');
        $today="INICT".date('ymd');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
...
public function CariIDPengguna(){
 ...
 $where = "";
 if(empty($id)){
 $where .= "WHERE UserName='Guest'" ;  
 }else{
 $where .= "WHERE UserName='$id'"  ;  
 }
        $t = "SELECT * FROM M_UserG5 $where";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			foreach($d->result() as $h){
...
date_default_timezone_set('Asia/Jakarta');
    ...
    public function DocNumDetailictOUTTR($id){
		$text = "SELECT max(DocNumDetail) as last FROM TD_RawMaterial WHERE DocNumDetail LIKE '%$id%'" ;
...
date_default_timezone_set('Asia/Jakarta');
    ...
    public function DocNumDetailictINTR($id){
		$text = "SELECT max(DocNumDetail) as last FROM TD_RawMaterial WHERE DocNumDetail LIKE '%$id%'" ;
...
date_default_timezone_set('Asia/Jakarta');
	...
	/**{{}}*/
    public function QtyICTTR_IN($kode,$tgl_awal,$tgl_akhir){
		$t = "SELECT ItemID,sum(QtyMat) as jml 
			FROM QTD_RawMaterial  
			WHERE ItemID='$kode' AND (DocDate BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."') AND IDTrcType=1010
			GROUP BY ItemID";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
	...
	/**{{}}**/
    public function QtyICTTR_OUT($kode,$tgl_awal,$tgl_akhir){
		$t = "SELECT ItemID,sum(QtyMat) as jml 
			FROM QTD_RawMaterial  
			WHERE ItemID='$kode' AND (DocDate BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."') AND IDTrcType=2020
			GROUP BY ItemID";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
	...
	//ict tambah docnumber out
    public function DocNumMaterialOUTICTOther(){
        //Time SERVER,
		date_default_timezone_set('Asia/Jakarta');
        $today="OUTICT".date('ymd');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
  ...
  public function DocNumDetailInSony($id){
		$text = "SELECT max(DocNumDetail) as last FROM TD_Trace WHERE DocNumDetail LIKE '%$id%' " ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
				$lastNoIN=$t->last;
                $lastNoUrut=substr($lastNoIN,11,4);
                $nextNoUrut=$lastNoUrut+1;
...
date_default_timezone_set('Asia/Jakarta');
       ...
       public function CariShift($id){
		$t = "SELECT * FROM M_Shift2 WHERE SysID='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			foreach($d->result() as $h){
       ...
       public function ambilBln2($bln){
		$exp = explode('-',$bln);
		$bln2 = $exp[1];
		return $bln2;
	} 
    
        public function ambilThn2($thn){
		$exp = explode('-',$thn);
		$thn2 = $exp[2];
		return $thn2;
	}
    
    public function CariJumlahSource($id){
        $t = "SELECT * FROM TD_Trace WHERE DocNumDetail='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
		foreach($d->result() as $h){
 ...
 public function DocNumDetailDelSony($id){
		$text = "SELECT max(DocNumDetail) as last FROM TD_Trace WHERE DocNumDetail LIKE '%$id%' " ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
 ...
 public function DocNumDetailMaterialOUTMPC2($id){
		$text = "SELECT max(DocNumDetail) as last FROM TD_Trace WHERE DocNumDetail LIKE '%$id%' " ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
    ...
    public function db_get_one($sql)
    {
		$hasil = '';
        $q = $this->db->query($sql);
        if($q->num_rows()>0)
...
public function FindNumOfUserRole($id){
 $text = "SELECT max(NumOf) as last FROM M_UserRole WHERE UserID='$id' " ;
 $data = $this->app_model->manualQuery($text);
 if($data->num_rows() > 0 ){
 foreach($data->result() as $t){
 $lastNoIN=$t->last;
 $nextNoUrut=$lastNoIN+1;
 $hasil = $nextNoUrut ; }
 }else{ $hasil = $nextNoUrut ; }
 return $hasil; }

public function FindNumOfUser(){
 $text = "SELECT max(RegID) as last FROM M_UserG5" ;
 $data = $this->app_model->manualQuery($text);
 if($data->num_rows() > 0 ){
 foreach($data->result() as $t){
 $lastNoIN=$t->last;
...
date_default_timezone_set('Asia/Jakarta');
 ...
 public function DocNumProdSony(){
 //Time SERVER,
date_default_timezone_set('Asia/Jakarta');
 $today="20".date('ymd');
 $text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
...
public function DocNumDetailProdSony($id){
 $text = "SELECT max(DocNumDetail) as last FROM TD_Trace WHERE DocNumDetail LIKE '%$id%' " ;
 $data = $this->app_model->manualQuery($text);
 if($data->num_rows() > 0 ){
 foreach($data->result() as $t){
 ...
 public function DocNumPOCustHead(){
 //Time SERVER,
 date_default_timezone_set('Asia/Jakarta');
 $today="SAI/PO/".date('y');
 $text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
 ...
 foreach($data->result() as $t){
 $lastNoINS=$t->last;
 $lastNoUrut=substr($lastNoINS,9,4);
 $nextNoUrut=$lastNoUrut+1;
 $nextNoTransaksi=$today.sprintf('%04s',$nextNoUrut);
 $hasil = $nextNoTransaksi; }
 }else{
...
public function DocNumPOCustDetail($id){
 $text = "SELECT max(DocNumDetail) as last FROM TD_Trace WHERE DocNumDetail LIKE '%$id%'" ;
 $data = $this->app_model->manualQuery($text);
 if($data->num_rows() > 0 ){
 foreach($data->result() as $t){
 ...
 return $hasil; }
	...
	public function DocNumDetailBPFGIN($id)
    {
        $text = "SELECT max(DocNumDetail) as last FROM TD_Transdetail WHERE DocNum LIKE '%$id%'" ;
		...
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
                ...
                $nextNoTransaksi=sprintf('%03s',$nextNoUrut);
				$hasil = $nextNoTransaksi;
                $hasil = sprintf('%03s',intval($t->last)+1);
			}
		}else{
			$hasil = $nextNoTransaksi ;
		}
    ...
    public function DocNumMaterialBPFGINOther()
    {
        date_default_timezone_set('Asia/Jakarta');
        $today="BPFGIN".date('ymd');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
    ...
    public function DocNumMaterialCreateBPFGOther()
    {
        date_default_timezone_set('Asia/Jakarta');
        $today="BPFG".date('ymd');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
    ...
    public function DocNumDetailCreateBPFG($id)
    {
        $text = "SELECT max(DocNumDetail) as last FROM TD_RawMaterial WHERE DocNum LIKE '%$id%'";        
        $data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
				$lastNoIN=$t->last;
    ...
    public function DocNumMaterialCreateSJGeneralOther()
    {
        $hasil = '';
        date_default_timezone_set('Asia/Jakarta');
        //$today="BPFG".date('ymd');
        $today="SAI/SJ/G/".date('y');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t)
            {
				$lastNoIN=$t->last;
                $lastNoUrut=substr($lastNoIN,-4,4);
                $nextNoUrut=$lastNoUrut+1;
                $nextNoTransaksi=$today.sprintf('%04s',$nextNoUrut);
				$hasil = $nextNoTransaksi;
			}
    ...
    public function DocNumMaterialCreateSJGAOther()
    {
        $hasil = '';
        date_default_timezone_set('Asia/Jakarta');
        //$today="BPFG".date('ymd');
        $today="SAI/SJ/G-GA/".date('y');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t)
    ...
    public function DocNumMaterialCreateSJAllOther($Gen)
    {
        $hasil = '';
        date_default_timezone_set('Asia/Jakarta');
        //$today="BPFG".date('ymd');
        $today="SAI/SJ/G-".$Gen."/".date('y');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t)
    ...
    public function DocNumMaterialCreateSJEngOther()
    {
        $hasil = '';
        date_default_timezone_set('Asia/Jakarta');
        //$today="BPFG".date('ymd');
        $today="SAI/SJ/G-ENG/".date('y');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t)
    ...
    public function DocNumDetailCreateSJ($id)
    {
        $text = "SELECT max(DocNumDetail) as last FROM TH_SuratJalan WHERE DocNum LIKE '%$id%'";        
        $data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
				$lastNoIN=$t->last;
    ...
    public function DocNumMaterialCreateSJSbcOther()
    {
        $hasil = '';
        date_default_timezone_set('Asia/Jakarta');
        //$today="BPFG".date('ymd');
        $today="SAI/SJ/G-SBC/".date('y');
		$text = "select max(DocNum) as last from G_DocNumMat WHERE DocNum LIKE '%$today%'" ;
		$data = $this->app_model->manualQuery($text);
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t)
