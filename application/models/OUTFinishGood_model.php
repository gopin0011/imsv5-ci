<?php class OUTFinishGood_model extends CI_Model{
function transaction_list(){
 $limit= 50; $offset = 0;
 return $this->db->query("SELECT * FROM QTH_RawMaterial WHERE IDTrcType=700 ORDER BY RegID DESC OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY"); }
...
function transaction_detail_2($id){
 return $this->db->query("SELECT * FROM QTD_RawMaterial WHERE DocNumDetail='$id' ORDER BY RegID DESC"); }
...
function transaction_detail_report($tgl1,$tgl2,$IDCust,$PartNo2){
 $where = " WHERE DocDate BETWEEN '$tgl1' AND '$tgl2' AND IDTrcType=700";
 if(empty($PartNo2)){
 if($IDCust!='semua'){ 
