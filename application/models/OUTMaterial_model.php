<?php class OUTMaterial_model extends CI_Model{
    
function transaction_list(){
 $limit= 50; $offset = 0;
 return $this->db->query("SELECT * FROM QTH_RawMaterial WHERE IDTrcType=200 ORDER BY RegID DESC OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY"); }
...
function List_Material(){
 return $this->db->query("SELECT * FROM ListMaterialOut WHERE IDTrcType=100 OR IDTrcType=105 ORDER BY RegID ASC "); }
...
$where = " WHERE DocDate BETWEEN '$tgl1' AND '$tgl2' AND IDTrcType=200";
 if(empty($PartNo)){
 if($IDCust!='semua'){
