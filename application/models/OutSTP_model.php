<?php class OutSTP_model extends CI_Model{
function transaction_list(){
 $limit= 50; $offset = 0;
 return $this->db->query("SELECT * FROM QTH_Production WITH (NOLOCK) WHERE IDTrcType=1000  ORDER BY RegID DESC OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY"); }
...
function transaction_detail_report($tgl1,$tgl2,$IDCust,$Factory,$PartNo,$IDLine){
 $where = " WHERE DocDate BETWEEN '$tgl1' AND '$tgl2' AND IDTrcType=1000";
 if(empty($PartNo)){
 if($IDCust!='ALL'){  
 if($Factory !='ALL'){
 if($IDLine!='ALL'){
